    ...
    <div className={`fixed inset-y-0 right-0 left-0 z-40 flex h-dvh min-h-0 flex-col overflow-hidden bg-background p-4 sm:p-6 ${isSidebarCollapsed ? 'lg:left-[72px]' : 'lg:left-[270px]'}`}>
      <div className="mx-auto flex min-h-0 w-full max-w-[1900px] flex-1 flex-col gap-4">
        ...
        <div className="flex flex-wrap items-center justify-between gap-3 border-t pt-4">
          ...
          <Button onClick={() => { setConfirmText(''); setConfirmOpen(true); }} disabled={importing || validRows === 0} className="font-bold"><Upload className="mr-2 size-4" /> {importing ? `Importando… ${progress}%` : `Importar ${validRows} proveedores`}</Button>
        </div>
      </div>

      <Dialog open={confirmOpen} onOpenChange={setConfirmOpen}>
        <DialogContent>
          <DialogHeader><DialogTitle>Confirmar importación</DialogTitle><DialogDescription>Se crearán {validRows} proveedores. Las filas con errores se omitirán y los avisos no bloquean la importación. Escribe IMPORTAR para continuar.</DialogDescription></DialogHeader>
      ...
      <Dialog open={importing} onOpenChange={() => undefined}>
        <DialogContent className="max-w-md [&>button]:hidden" onInteractOutside={(event) => event.preventDefault()} onEscapeKeyDown={(event) => event.preventDefault()}>
          <div className="flex flex-col items-center gap-5 py-5 text-center"><div className="relative flex size-24 items-center justify-center rounded-full border-4 border-primary/20 bg-primary/5"><div className="absolute inset-0 animate-spin rounded-full border-4 border-transparent border-t-primary" /><span className="text-xl font-black text-primary">{progress}%</span></div><div><DialogTitle className="text-xl">Importando proveedores</DialogTitle><DialogDescription className="mt-2">Validando y guardando los registros. No cierres esta ventana.</DialogDescription></div><div className="h-3 w-full overflow-hidden rounded-full bg-muted"><div className="h-full rounded-full bg-primary transition-all duration-300" style={{ width: `${Math.max(progress, 3)}%` }} /></div></div>
        </DialogContent>
      </Dialog>
