...
import { 
  PackageCheck, Plus, Search, Eye, Trash2, CheckCircle2, Clock, ChevronLeft
} from 'lucide-react';
                ...
                <Button variant="outline" className="rounded-xl border-rose-500/50 text-rose-500 hover:bg-rose-500 hover:text-white font-black uppercase text-[10px] tracking-widest px-4"
                  onClick={async () => {
                     if(confirm('¿Seguro que deseas eliminar?')){
                         try { await purchaseReceiptsService.delete(editingId); toast.success('Eliminado'); setEditingId(null); onRefresh(); } catch { toast.error('Error'); }
                     }
                  }}>
              ...
              {(localDoc.items || []).map((item: any, idx: number) => (
                <div key={item.id || idx} className="grid grid-cols-12 gap-2 items-center">
                  <div className="col-span-6">
                    <Input value={item.description || ''} onChange={(e) => handleItemChange(idx, 'description', e.target.value)} className="h-8 text-xs font-bold text-primary" placeholder="Ej. Llantas Michelin" />
                  </div>
                  <div className="col-span-2">
                    <Input type="number" min="0" value={item.quantityOrdered === 0 ? '' : item.quantityOrdered} onChange={(e) => handleItemChange(idx, 'quantityOrdered', e.target.value)} className="h-8 text-xs text-right bg-muted/20" placeholder="0" />
                  </div>
                  <div className="col-span-2">
                    <Input type="number" min="0" value={item.quantityReceived === 0 ? '' : item.quantityReceived} onChange={(e) => handleItemChange(idx, 'quantityReceived', e.target.value)} className="h-8 text-xs text-right font-bold text-emerald-500 border-emerald-500/50" placeholder="0" />
                  </div>
        ...
        <EditableDataTable data={filtered} columns={columns} onRowUpdate={handleUpdate} isLoading={loading}
          ...
          actions={(row) => (
            <div className="flex gap-1">
              ...
              <Button title="Eliminar" variant="ghost" size="icon" className="size-8 rounded-lg hover:bg-rose-500/10 hover:text-rose-500" onClick={async () => { try { await purchaseReceiptsService.delete(row.id); onRefresh(); } catch { toast.error('Error'); } }}><Trash2 className="size-4" /></Button>
            </div>
          )}
        />
      </div>
