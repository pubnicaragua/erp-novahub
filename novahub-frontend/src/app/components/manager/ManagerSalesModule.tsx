...
export function ManagerSalesModule({ view, onViewChange, groupId, businessUnitId, branchId, branches, reportCurrency, onEnterBranch, enterableBranchIds = new Set<string>(), canExport = true }: { view: ManagerSalesView; onViewChange: (view: ManagerSalesView) => void; groupId: string; businessUnitId?: string; branchId?: string; branches: BranchOption[]; reportCurrency: string; onEnterBranch?: (groupId: string, branchId: string) => Promise<void>; enterableBranchIds?: Set<string>; canExport?: boolean }) {
  ...
  const selectedSalesDocumentBranchId = String(salesDocumentDetail?.document?.branchId || selectedSalesDocument?.row?.branchId || '');
  const selectedDeliveryBranchId = String((deliveryDetail?.delivery || selectedDelivery)?.billingTenantId || (deliveryDetail?.delivery || selectedDelivery)?.branchId || '');
  const selectedCashBranchId = String((cashSessionDetail?.session || selectedCashSession)?.branchId || '');
  const selectedPriceListBranchId = String((priceListDetail?.priceList || selectedPriceList)?.branchId || '');
  ...
  return <div className="sales-module min-w-0 space-y-5 overflow-x-hidden p-4 sm:p-6 md:p-8">
    <div className="flex min-w-0 flex-col gap-4 md:flex-row md:items-start md:justify-between">
      <div className="flex min-w-0 items-start gap-3">
        ...
        <div className="min-w-0"><h1 className="truncate text-3xl font-black uppercase italic tracking-tighter sm:text-4xl">{isInvoiceSeriesView ? 'Facturación y reportes' : 'Ventas'}</h1><Badge variant="outline" className="mt-3 rounded-md border-primary/20 bg-primary/10 text-[10px] font-black uppercase tracking-widest text-primary">{branches.length} sucursal(es) en el alcance</Badge></div>
      </div>
      <div className="flex shrink-0 flex-wrap gap-2">
        ...
        {!isInvoiceSeriesView && canExport && (view === 'quotes' ? <DropdownMenu>
    ...
    {view === 'invoice-series' ? <ManagerInvoiceSeriesSettings groupId={groupId} businessUnitId={businessUnitId} branchId={branchId} /> : view === 'overview' ? <SalesOverview metrics={metrics} reportCurrency={activeReportCurrency} /> : <>
      ...
      <SalesDocumentDetailSheet
        ...
        onDownloadPdf={canExport ? (format) => { void downloadQuotePdf(format); } : undefined}
        onGoToBranch={onEnterBranch && enterableBranchIds.has(selectedQuoteBranchId) ? () => { void onEnterBranch(groupId, selectedQuoteBranchId); } : undefined}
      />
      <SalesDocumentDetailSheet
        ...
        onDownloadPdf={canExport ? (format) => { void downloadSalesDocumentPdf(format); } : undefined}
        onGoToBranch={onEnterBranch && enterableBranchIds.has(selectedSalesDocumentBranchId) ? () => { void onEnterBranch(groupId, selectedSalesDocumentBranchId); } : undefined}
      ...
      />
      <ManagerSalesDeliverySheet open={Boolean(selectedDelivery)} onOpenChange={(open) => { if (!open) { setSelectedDelivery(null); setDeliveryDetail(null); } }} loading={deliveryDetailLoading} delivery={deliveryDetail?.delivery || selectedDelivery} history={deliveryDetail?.history} reportCurrency={activeReportCurrency} onDownload={canExport && deliveryDetail?.delivery ? () => { void exportManagerSalesPdf({ rows: [exportRow('deliveries', deliveryDetail.delivery, activeReportCurrency, displayMode)], tenantName: themeConfig?.tenantName || user?.tenantName || 'Empresa', tenantLogo: themeConfig?.logo, primaryColor: themeConfig?.colors?.primary, title: 'Detalle de entrega', fileBase: `detalle_entrega_${deliveryDetail.delivery.number || 'sin_numero'}`, reportCurrency: activeReportCurrency, metrics, pdfDesign: deliveryDetail.delivery.pdfDesign }); } : undefined} onGoToBranch={onEnterBranch && enterableBranchIds.has(selectedDeliveryBranchId) ? (targetBranchId) => { if (enterableBranchIds.has(targetBranchId)) void onEnterBranch(groupId, targetBranchId); } : undefined} onWhatsApp={deliveryDetail?.delivery?.customerPhone ? () => { const whatsappUrl = buildCustomerWhatsAppUrl(deliveryDetail.delivery.customerPhone, `Hola ${deliveryDetail?.delivery?.customerName || 'cliente'}, te compartimos la información de tu entrega ${deliveryDetail?.delivery?.number || ''}.`); if (!whatsappUrl) { toast.error('El teléfono de la entrega no está normalizado en E.164.'); return; } window.open(whatsappUrl, '_blank', 'noopener,noreferrer'); } : undefined} />
      <ManagerSalesCashSheet open={Boolean(selectedCashSession)} onOpenChange={(open) => { if (!open) { setSelectedCashSession(null); setCashSessionDetail(null); } }} loading={cashSessionDetailLoading} session={cashSessionDetail?.session || selectedCashSession} invoices={cashSessionDetail?.invoices} log={cashSessionDetail?.log} reportCurrency={activeReportCurrency} onDownload={canExport && cashSessionDetail?.session ? () => { void exportManagerSalesPdf({ rows: [exportRow('cash', cashSessionDetail.session, activeReportCurrency, displayMode)], tenantName: themeConfig?.tenantName || user?.tenantName || 'Empresa', tenantLogo: themeConfig?.logo, primaryColor: themeConfig?.colors?.primary, title: 'Detalle de sesión de caja', fileBase: `detalle_sesion_caja_${cashSessionDetail.session.register?.code || 'sin_caja'}`, reportCurrency: activeReportCurrency, metrics, pdfDesign: cashSessionDetail.session.pdfDesign }); } : undefined} onGoToBranch={onEnterBranch && enterableBranchIds.has(selectedCashBranchId) ? (targetBranchId) => { if (enterableBranchIds.has(targetBranchId)) void onEnterBranch(groupId, targetBranchId); } : undefined} />
      <ManagerSalesPriceListSheet open={Boolean(selectedPriceList)} onOpenChange={(open) => { if (!open) { setSelectedPriceList(null); setPriceListDetail(null); } }} loading={priceListDetailLoading} priceList={priceListDetail?.priceList || selectedPriceList} items={priceListDetail?.items} reportCurrency={activeReportCurrency} onDownload={canExport && priceListDetail?.priceList ? () => { void exportManagerSalesPdf({ rows: (priceListDetail.items || []).map((item: any) => exportRow('pricelists', { ...item, priceListId: priceListDetail.priceList.id, branchName: priceListDetail.priceList.branchName }, activeReportCurrency, displayMode)), tenantName: themeConfig?.tenantName || user?.tenantName || 'Empresa', tenantLogo: themeConfig?.logo, primaryColor: themeConfig?.colors?.primary, title: `Lista ${priceListDetail.priceList.name}`, fileBase: `lista_de_precios_${priceListDetail.priceList.code || priceListDetail.priceList.name || 'sin_codigo'}`, reportCurrency: activeReportCurrency, filterSummary: quoteFilterSummary, metrics, pdfDesign: priceListDetail.priceList.pdfDesign }); } : undefined} onGoToBranch={onEnterBranch && enterableBranchIds.has(selectedPriceListBranchId) ? (targetBranchId) => { if (enterableBranchIds.has(targetBranchId)) void onEnterBranch(groupId, targetBranchId); } : undefined} />
    </>}
  </div>;
}
...
function ManagerSalesRecordCard({ view, row, showBranch: _showBranch, reportCurrency, onDetail, openingId, priceListMode = 'lists' }: { view: ManagerSalesView; row: any; showBranch: boolean; reportCurrency: string; onDetail?: (row: any) => void; openingId?: string | number | null; priceListMode?: 'lists' | 'prices' }) {
          ...
          : view === 'creditnotes'
            ...
            : view === 'credits'
              ...
              : view === 'deliveries'
                ...
                : view === 'cash'
                  ? [['Apertura', formatDate(row.openedAt, true)], ['Cierre', formatDate(row.closedAt, true)], ['Facturas', formatNumber(row.invoiceCount)], ['Diferencia', <MoneyPair row={row} original={row.differenceNIO} equivalent={row.reportDifferenceNIO} reportCurrency={reportCurrency} compact />]]
                  : view === 'pricelists'
  ...
  return <Card role={onDetail ? 'button' : undefined} tabIndex={onDetail ? 0 : undefined} aria-label={onDetail ? `Ver detalle de ${title}` : undefined} onClick={() => onDetail?.(row)} onKeyDown={handleKeyDown} className={cn('rounded-2xl border-border/60 shadow-sm', onDetail && 'cursor-pointer transition-all hover:-translate-y-0.5 hover:border-primary/40 hover:bg-primary/5 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary/40')}>
    <CardContent className="space-y-4 p-5">
      <p className="border-b border-border/60 pb-2 text-xs font-semibold text-primary">Sucursal: {row.branchName || 'Sucursal no identificada'}</p>
      <div className="flex items-start justify-between gap-3"><div className="min-w-0"><p className="truncate font-mono text-base font-black text-primary">{title}</p></div><div className="flex shrink-0 items-center gap-2">{isOpening && <span role="status" className="inline-flex items-center gap-1 rounded-lg bg-primary/10 px-2 py-1 text-[9px] font-black uppercase tracking-wider text-primary"><Loader2 className="size-3 animate-spin" /> Abriendo…</span>}{statusBadge(status)}</div></div>
...
function tableColumns(view: ManagerSalesView, showBranch: boolean, reportCurrency: string, priceListMode: 'lists' | 'prices' = 'lists', displayMode: CurrencyDisplayMode = 'DEFAULT'): TableColumn[] {
  const branch: TableColumn[] = showBranch ? [{ label: 'Sucursal', sortKey: 'branchName', sortType: 'text', render: (row: any) => <Badge variant="secondary" className="max-w-44 truncate">{row.branchName || 'Sucursal no identificada'}</Badge> }] : [];
  const prependBranch = (columns: TableColumn[]) => showBranch ? [...branch, ...columns] : columns;
  ...
  if (view === 'customers') return prependBranch([
    { label: 'Código', sortKey: 'code', sortType: 'text', render: (row) => <span className="font-mono font-bold">{customerValue(row.code, row.id?.slice(0, 8) || '—')}</span> },
  ...
  if (view === 'quotes') return prependBranch([
    { label: 'Número', sortKey: 'number', sortType: 'text', render: (row) => <span className="font-mono font-bold text-primary">{row.number || '—'}</span> },
  ...
  const documentNumber = (row: any, fallback = 'Sin número') => <span className="font-mono font-bold text-primary">{row.number || fallback}</span>;
...
function ManagerCustomerDetailSheet({ groupId, customer, reportCurrency, onOpenChange, onEnterBranch, enterableBranchIds, onExportHistory, exportingHistory = false }: { groupId: string; customer: any | null; reportCurrency: string; onOpenChange: (open: boolean) => void; onEnterBranch?: (groupId: string, branchId: string) => Promise<void>; enterableBranchIds: Set<string>; onExportHistory?: () => void; exportingHistory?: boolean }) {
...
function cashSummaryExportRow(row: any, reportCurrency: string, displayMode: CurrencyDisplayMode = 'DEFAULT') {
  const amount = (original: unknown, equivalent: unknown) => displayMode === 'ORIGINAL' || equivalent == null
    ...
    : formatMoney(equivalent, reportCurrency, true);
  return { Sucursal: row.branchName || 'Sucursal no identificada', Caja: row.registerName, Código: row.registerCode, Sesiones: row.sessions, Facturas: row.invoices, Facturado: amount(row.billed, row.reportBilled), Esperado: amount(row.expectedAmountNIO, row.reportExpectedAmountNIO), Diferencia: amount(row.differenceNIO, row.reportDifferenceNIO) };
}
...
function exportRow(view: ManagerSalesView, row: any, reportCurrency: string, displayMode: CurrencyDisplayMode = 'DEFAULT') {
  ...
  const base = { Fecha: formatDate(row.date || row.openedAt || row.nextInvoiceDate), Estado: statusLabel(row.status || row.deliveryStatus), ...rateContext };
  ...
  if (view === 'customers') return { Sucursal: row.branchName || 'Sucursal no identificada', Código: row.code, Cliente: row.name, Tipo: customerTypeLabel(row.type), Cédula: row.taxId, RUC: row.ruc, 'Régimen fiscal': row.fiscalRegime, Correo: row.email, Teléfono: row.phone, Departamento: row.department, Estado: customerStatusInfo(row.status).label };
  if (view === 'payments') return { Sucursal: row.branchName || 'Sucursal no identificada', Pago: row.number, Cliente: row.customerName, Método: statusLabel(row.method), Documento: row.documentNumber, [amountLabel]: displayAmount(row.amount, row.reportAmount), ...base };
  if (view === 'cash') return { Sucursal: row.branchName || 'Sucursal no identificada', Caja: row.registerName, Código: row.registerCode, Apertura: formatDate(row.openedAt, true), Cierre: formatDate(row.closedAt, true), 'Abrió': row.openedByName, 'Cerró': row.closedByName, Facturas: row.invoiceCount, Diferencia: displayMode === 'ORIGINAL' ? formatMoney(row.differenceNIO, 'NIO', true) : displayAmount(row.differenceNIO, row.reportDifferenceNIO), ...base };
  if (view === 'deliveries') return { Sucursal: row.billingBranchName || row.branchName || 'Sucursal no identificada', Entrega: row.number, Cliente: row.customerName, Facturación: row.billingBranchName, 'Entrega en': row.deliveryBranchName, Factura: row.invoiceNumber, Artículos: row.itemCount, Cobro: statusLabel(row.paymentStatus), [amountLabel]: displayAmount(row.total, row.reportTotal), ...base };
  if (view === 'pricelists') return row.productName ? { Sucursal: row.branchName || 'Sucursal no identificada', Lista: row.listName, Código: row.listCode, SKU: row.productCode, Producto: row.productName, Variante: row.variantName, Precio: displayAmount(row.price, row.reportPrice), 'Costo referencia': formatMoney(row.costPrice, currency, true), 'Tasa aplicada': row.reportRateLabel || '—', Estado: statusLabel(row.listActive ? 'ACTIVE' : 'INACTIVE'), Actualizada: formatDate(row.updatedAt) } : { Sucursal: row.branchName || 'Sucursal no identificada', Código: row.code, Lista: row.name, Productos: row.itemCount, 'Precios en': row.currencies?.join(' / ') || row.currency, Predeterminada: row.isDefault ? 'Sí' : 'No', Estado: statusLabel(row.isActive ? 'ACTIVE' : 'INACTIVE'), Actualizada: formatDate(row.updatedAt) };
  return { Sucursal: row.branchName || 'Sucursal no identificada', Número: row.number, Cliente: row.customerName, Total: displayAmount(row.total, row.reportTotal), 'Cargos adicionales': additionalChargesLabel(row), Saldo: row.balance == null ? '—' : displayAmount(row.balance, row.reportBalance), ...base };
}
