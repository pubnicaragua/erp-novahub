  ...
  return (
    <Card className="overflow-hidden rounded-3xl border-primary/20 bg-gradient-to-br from-primary/[0.06] via-card to-card shadow-sm">
      ...
      <CardContent className="space-y-5 p-4 sm:p-6">
        ...
        <div className="grid gap-4 lg:grid-cols-[1.1fr_0.9fr]">
          <div className="rounded-2xl border border-border/60 bg-background/60 p-4">
            ...
            <div className="mt-3 grid gap-2 sm:grid-cols-2">
              ...
              {source && !targetBranches.length && <p className="text-xs text-warning">La sucursal de origen no tiene rubro configurado.</p>}
            </div>
          ...
          <div className="rounded-2xl border border-border/60 bg-background/60 p-4">
            ...
            <div className="mt-3 grid grid-cols-2 gap-2 sm:grid-cols-3 lg:grid-cols-2">
              ...
              <div className={`rounded-xl p-3 ${validationErrors.length ? 'bg-warning/10' : 'bg-success/10'}`}><p className="text-[10px] text-muted-foreground">Observaciones</p><p className={`mt-1 text-xl font-black ${validationErrors.length ? 'text-warning' : 'text-success'}`}>{validationErrors.length}</p></div>
        ...
        {validationErrors.length > 0 && <div className="rounded-2xl border border-warning/30 bg-warning/[0.08] p-3 text-xs text-warning dark:text-warning"><p className="flex items-center gap-2 font-black uppercase tracking-widest"><AlertTriangle className="size-4" /> Corrige la plantilla antes de importar</p><ul className="mt-2 grid gap-1 sm:grid-cols-2">{validationErrors.slice(0, 8).map((error) => <li key={error}>• {error}</li>)}</ul>{validationErrors.length > 8 && <p className="mt-1">Y {validationErrors.length - 8} observación(es) más.</p>}</div>}
