...
const ManagerMoney = ({ value, currency, selectedAmount, selectedCurrency, sourceExchangeRate }: { value: unknown; currency?: string | null; selectedAmount?: number | null; selectedCurrency?: string | null; sourceExchangeRate?: number | null }) => <CurrencyDisplayAmount amount={Number(value || 0)} sourceCurrency={currency || 'NIO'} sourceExchangeRate={sourceExchangeRate || undefined} selectedAmount={selectedAmount} selectedCurrency={selectedCurrency || undefined} />;
...
function SheetCloseFooter({ onClose }: { onClose: (open: boolean) => void }) {
  return <div className="flex shrink-0 justify-end border-t border-border/60 bg-background/95 px-5 py-3 backdrop-blur-md sm:px-7">
    <Button type="button" variant="outline" className="min-w-24 rounded-xl" onClick={() => onClose(false)}>Cerrar</Button>
  </div>;
}
  ...
  return <Sheet open={open} onOpenChange={onOpenChange}><SheetContent side="right" className="erp-detail-panel erp-detail-panel--medium w-full overflow-hidden p-0">
    <ScrollArea className="min-h-0 flex-1"><div className="space-y-5 p-5 sm:p-7">
      ...
      {loading ? <LoadingDetail /> : delivery ? <>
        <Card className="rounded-2xl border-primary/20 bg-primary/[0.04] p-4"><div className="grid grid-cols-2 gap-4 sm:grid-cols-4"><Field label="Cliente" value={delivery.customerName} icon={UserRound} /><Field label="Artículos" value={formatNumber(delivery.itemCount)} icon={Package} /><Field label="Cobro" value={statusBadge(delivery.paymentStatus)} icon={WalletCards} /><Field label="Total" value={<ManagerMoney value={delivery.total} currency={delivery.currency} selectedAmount={delivery.reportTotal} selectedCurrency={reportCurrency} sourceExchangeRate={delivery.exchangeRate} />} icon={Receipt} /></div></Card>
      ...
      <DetailSection title="Artículos" icon={Package}>{delivery.items?.length ? <div className="space-y-2">{delivery.items.map((item: any) => <div key={item.id} className="flex items-start justify-between gap-3 rounded-xl border border-border/50 bg-background/30 p-3 text-sm"><div className="min-w-0"><p className="font-semibold">{item.productName || item.description}</p><p className="mt-0.5 flex flex-wrap items-center gap-1 text-xs text-muted-foreground">{item.productCode || 'Sin código'} · {formatNumber(item.quantity)} × <ManagerMoney value={item.unitPrice} currency={delivery.currency} sourceExchangeRate={delivery.exchangeRate} /></p></div><span className="shrink-0 font-black"><ManagerMoney value={item.total} currency={delivery.currency} sourceExchangeRate={delivery.exchangeRate} /></span></div>)}</div> : <p className="text-sm text-muted-foreground">No hay artículos registrados.</p>}</DetailSection>
    ...
    </div></ScrollArea><SheetCloseFooter onClose={onOpenChange} />
  </SheetContent></Sheet>;
}
...
export function ManagerSalesCashSheet({ open, onOpenChange, loading, session, invoices = [], log = [], reportCurrency, onDownload, onGoToBranch }: CommonSheetProps & { session: any | null; invoices?: any[]; log?: any[]; reportCurrency: string }) {
  return <Sheet open={open} onOpenChange={onOpenChange}><SheetContent side="right" className="erp-detail-panel erp-detail-panel--medium w-full overflow-hidden p-0"><ScrollArea className="min-h-0 flex-1"><div className="space-y-5 p-5 sm:p-7">
    ...
    {loading ? <LoadingDetail /> : session ? <>
      ...
      <DetailSection title="Apertura y cierre" icon={Clock3}><div className="grid grid-cols-1 gap-4 sm:grid-cols-2"><Field label="Apertura" value={formatDate(session.openedAt, true)} icon={CalendarDays} /><Field label="Cierre" value={formatDate(session.closedAt, true)} icon={CalendarDays} /><Field label="Abrió" value={session.openedBy?.name || session.openedBy?.email} icon={UserRound} /><Field label="Cerró" value={session.closedBy?.name || session.closedBy?.email} icon={UserRound} /><Field label="Monto inicial" value={<ManagerMoney value={session.initialAmountNIO} currency="NIO" selectedAmount={session.reportInitialAmountNIO} selectedCurrency={reportCurrency} sourceExchangeRate={session.exchangeRateUSD || session.reportRate} />} icon={Banknote} /><Field label="Monto esperado" value={<ManagerMoney value={session.expectedAmountNIO} currency="NIO" selectedAmount={session.reportExpectedAmountNIO} selectedCurrency={reportCurrency} sourceExchangeRate={session.exchangeRateUSD || session.reportRate} />} icon={Banknote} /><Field label="Monto final" value={<ManagerMoney value={session.finalAmountNIO} currency="NIO" selectedAmount={session.reportFinalAmountNIO} selectedCurrency={reportCurrency} sourceExchangeRate={session.exchangeRateUSD || session.reportRate} />} icon={Banknote} /><Field label="Tasa aplicada" value={`${session.reportRateLabel || '—'} · ${formatCurrencyDescriptor(reportCurrency)}`} icon={WalletCards} /></div></DetailSection>
      <DetailSection title="Facturas de la sesión" icon={Receipt}>{invoices.length ? <div className="space-y-2">{invoices.map((invoice: any) => <div key={invoice.id} className="flex items-start justify-between gap-3 rounded-xl border border-border/50 bg-background/30 p-3 text-sm"><div className="min-w-0"><p className="font-mono font-black text-primary">{invoice.number}</p><p className="mt-0.5 truncate text-xs text-muted-foreground">{invoice.customerName} · {formatDate(invoice.date)}</p><p className="mt-1 text-xs">{statusText(invoice.status)} · {statusText(invoice.paymentMethod)}</p></div><div className="shrink-0 text-right"><p className="font-black"><ManagerMoney value={invoice.total} currency={invoice.currency} selectedAmount={invoice.reportTotal} selectedCurrency={reportCurrency} sourceExchangeRate={invoice.exchangeRate} /></p></div></div>)}</div> : <p className="text-sm text-muted-foreground">No hay facturas asociadas.</p>}</DetailSection>
  ...
  </div></ScrollArea><SheetCloseFooter onClose={onOpenChange} /></SheetContent></Sheet>;
}
...
export function ManagerSalesPriceListSheet({ open, onOpenChange, loading, priceList, items = [], reportCurrency, onDownload, onGoToBranch }: CommonSheetProps & { priceList: any | null; items?: any[]; reportCurrency: string }) {
  return <Sheet open={open} onOpenChange={onOpenChange}><SheetContent side="right" className="erp-detail-panel erp-detail-panel--medium w-full overflow-hidden p-0"><ScrollArea className="min-h-0 flex-1"><div className="space-y-5 p-5 sm:p-7">
    ...
    {loading ? <LoadingDetail /> : priceList ? <>
      ...
      <DetailSection title="Precios de la lista" icon={Package}><div className="mb-3 rounded-xl border border-border/50 bg-muted/20 p-3 text-xs text-muted-foreground">Moneda de reporte: <span className="font-black text-primary">{formatCurrencyDescriptor(reportCurrency)}</span>. Se conserva la moneda original de cada precio.</div>{items.length ? <div className="space-y-2">{items.map((item: any) => <div key={item.id} className="flex items-start justify-between gap-3 rounded-xl border border-border/50 bg-background/30 p-3 text-sm"><div className="min-w-0"><p className="font-semibold">{item.productName}</p><p className="mt-0.5 text-xs text-muted-foreground">{item.productCode}{item.variantName ? ` · ${item.variantName}` : ''}</p><p className="mt-1 text-xs text-muted-foreground">Costo de referencia: <ManagerMoney value={item.costPrice} currency={item.currency} sourceExchangeRate={item.exchangeRate} /></p></div><div className="shrink-0 text-right"><p className="font-black"><ManagerMoney value={item.price} currency={item.currency} selectedAmount={item.reportPrice} selectedCurrency={reportCurrency} sourceExchangeRate={item.exchangeRate} /></p></div></div>)}</div> : <p className="text-sm text-muted-foreground">Esta lista no tiene productos asociados.</p>}</DetailSection>
