  ...
  const save = async (item: ManagerInvoiceSeriesItem, reset = false) => {
    ...
    try {
      await enterpriseGroupsService.saveManagerInvoiceSeriesConfiguration(groupId, {
        ...
        businessUnitId,
        scopeBranchId: item.branchId || null,
    ...
    <div className="manager-invoice-series min-w-0 space-y-5 overflow-x-hidden">
      <Card className="overflow-hidden rounded-3xl border-border/60 shadow-sm">
        ...
        <CardContent className="space-y-5 p-4 sm:p-6">
          <div className="flex min-w-0 flex-col gap-3 rounded-2xl border border-sky-500/20 bg-sky-500/[0.06] p-4 text-sm sm:flex-row sm:items-start"><Info className="mt-0.5 size-4 shrink-0 text-sky-600" /><p className="min-w-0 leading-5 text-muted-foreground">Selecciona una sucursal del grupo y configura sus alcances de facturación. Los prefijos son únicos entre sucursales y no modifican facturas ya emitidas.</p></div>
