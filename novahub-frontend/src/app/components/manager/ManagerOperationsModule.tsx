...
import { Activity, Banknote, CalendarDays, ChevronLeft, ChevronRight, Cloud, Download, Eye, FolderKanban, LayoutDashboard, ListChecks, Loader2, MessageCircle, Package, RefreshCw, Search, Scale, Ticket, Utensils, Wrench, X } from 'lucide-react';
...
const MODULES: Array<{ id: OperationsModule; label: string; icon: typeof Activity; views: Array<{ id: string; label: string }> }> = [
  { id: 'activities', label: 'Actividades', icon: Activity, views: [{ id: 'overview', label: 'Resumen' }, { id: 'activities', label: 'Actividades' }, { id: 'calendar', label: 'Calendario' }, { id: 'meetings', label: 'Reuniones' }, { id: 'reminders', label: 'Recordatorios' }, { id: 'logs', label: 'Bitácora' }] },
  { id: 'projects', label: 'Proyectos', icon: FolderKanban, views: [{ id: 'overview', label: 'Resumen' }, { id: 'projects', label: 'Proyectos' }, { id: 'tasks', label: 'Tareas' }, { id: 'milestones', label: 'Hitos' }, { id: 'costs', label: 'Costos' }, { id: 'budget', label: 'Presupuesto' }, { id: 'resources', label: 'Recursos' }, { id: 'documents', label: 'Documentos' }, { id: 'activities', label: 'Actividades' }] },
  { id: 'tickets', label: 'Tickets', icon: Ticket, views: [{ id: 'overview', label: 'Resumen' }, { id: 'tickets', label: 'Tickets' }, { id: 'categories', label: 'Categorías' }, { id: 'faqs', label: 'Base de conocimiento' }, { id: 'agents', label: 'Agentes' }] },
  { id: 'documents', label: 'Documentos', icon: Cloud, views: [{ id: 'overview', label: 'Resumen' }, { id: 'documents', label: 'Archivos' }, { id: 'reports', label: 'Reportes guardados' }] },
  { id: 'restaurant', label: 'Restaurante', icon: Utensils, views: [{ id: 'overview', label: 'Resumen' }, { id: 'orders', label: 'Comandas' }, { id: 'kitchen', label: 'Cocina' }, { id: 'tables', label: 'Mesas' }, { id: 'menu', label: 'Carta' }, { id: 'reports', label: 'Reportes' }] },
  { id: 'logistics', label: 'Logística', icon: Package, views: [{ id: 'overview', label: 'Resumen' }, { id: 'shipments', label: 'Envíos' }, { id: 'packages', label: 'Paquetes' }, { id: 'batches', label: 'Recepciones' }, { id: 'reconciliation', label: 'Conciliación' }, { id: 'billing', label: 'Facturación' }] },
  { id: 'financing', label: 'Financiamiento PYME', icon: Banknote, views: [{ id: 'overview', label: 'Resumen' }, { id: 'applications', label: 'Solicitudes' }] },
  { id: 'legal', label: 'Asesoría legal', icon: Scale, views: [{ id: 'overview', label: 'Resumen' }, { id: 'cases', label: 'Casos' }, { id: 'reminders', label: 'Recordatorios' }] },
  { id: 'novachat', label: 'NovaChat', icon: MessageCircle, views: [{ id: 'overview', label: 'Resumen' }, { id: 'conversations', label: 'Conversaciones' }, { id: 'channels', label: 'Canales' }, { id: 'contacts', label: 'Contactos' }, { id: 'agents', label: 'Agentes' }] },
  { id: 'support', label: 'Soporte técnico', icon: Wrench, views: [{ id: 'overview', label: 'Resumen' }, { id: 'tickets', label: 'Tickets' }] },
];
...
const formatStatus = (value: unknown) => String(value || '—').replaceAll('_', ' ');
...
const STATUS_LABELS: Record<string, string> = { OPEN: 'Abierto', IN_PROGRESS: 'En proceso', RESOLVED: 'Resuelto', CLOSED: 'Cerrado', PENDING: 'Pendiente', COMPLETED: 'Completado', CANCELLED: 'Cancelado', DRAFT: 'Borrador', PLANNED: 'Planificado', PAUSED: 'En pausa', WAITING_DOCS: 'Esperando documentos', APPROVED: 'Aprobado', REJECTED: 'Rechazado', DISBURSED: 'Desembolsado', PENDING_CONFIRMATION: 'Pendiente de confirmación', CONFIRMED: 'Confirmado', SENT_TO_KITCHEN: 'Enviado a cocina', IN_PREPARATION: 'En preparación', READY: 'Listo', SERVED: 'Servido', PARTIALLY_PAID: 'Pago parcial', PAID: 'Pagado', AVAILABLE: 'Disponible', OCCUPIED: 'Ocupada', RESERVED: 'Reservada', CLEANING: 'Limpieza', INACTIVE: 'Inactiva', ACTIVE: 'Activo', RECEIVED: 'Recibido', CUSTOMS: 'Aduana', OUT_FOR_DELIVERY: 'En reparto', DELIVERED: 'Entregado', RETURNED: 'Devuelto', ON_HOLD: 'En espera', LOST: 'Extraviado', NONE: 'Pendiente', PURCHASED: 'Conciliado', AVAILABLE_FOR_BILLING: 'Disponible para facturar', BILLED: 'Facturado', PUBLISHED: 'Publicado', ARCHIVED: 'Archivado' };
...
export function ManagerOperationsModule({ module, onModuleChange, allowedModules, groupId, businessUnitId, branchId, branches }: { module: OperationsModule; onModuleChange: (module: OperationsModule) => void; allowedModules?: OperationsModule[]; groupId: string; businessUnitId?: string; branchId?: string; branches: BranchOption[] }) {
  ...
  const visibleModules = MODULES.filter((item) => !allowedModules || allowedModules.includes(item.id));
  const definition = visibleModules.find((item) => item.id === module) || visibleModules[0] || MODULES[0];
  ...
  const changeModule = (next: OperationsModule) => {
  ...
  const changeView = (next: string) => { setView(next); setPage(1); setStatus(''); setSelectedRow(null); };
  const clearFilters = () => { setSearch(''); setStatus(''); setDateFrom(''); setDateTo(''); setPage(1); };
  const exportReport = async () => {
    setExporting(true);
    try {
      const report = await enterpriseGroupsService.getOperationsModule(groupId, { module: definition.id, view: activeView, businessUnitId, branchId, search: search || undefined, status: status || undefined, dateFrom: dateFrom || undefined, dateTo: dateTo || undefined, page: 1, pageSize: 5000, report: true });
      const exportRows = (report.data || []).map((row: any) => ({ Registro: primaryOf(row), Detalle: detailOf(row), Fecha: formatDate(dateOf(row)), Estado: formatStatus(row.status), Valor: amountOf(row) == null ? '' : formatValue(amountOf(row)), Sucursal: row.branchName || '', Rubro: row.businessUnitName || '' }));
  ...
  return <div className="manager-operations-module min-w-0 space-y-5 overflow-x-hidden p-4 sm:p-6 md:p-8">
    <div className="flex min-w-0 flex-col gap-4 md:flex-row md:items-start md:justify-between">
      <div className="flex min-w-0 items-start gap-3"><div className="flex size-12 shrink-0 items-center justify-center rounded-2xl bg-primary/10 text-primary"><definition.icon className="size-6" /></div><div className="min-w-0"><h1 className="truncate text-3xl font-black uppercase italic tracking-tighter sm:text-4xl">Operaciones <span className="text-primary">consolidadas</span></h1><p className="mt-1 text-sm text-muted-foreground">Lectura agregada por sucursal, con origen visible en cada registro.</p><Badge variant="outline" className="mt-3 rounded-md border-primary/20 bg-primary/10 text-[10px] font-black uppercase tracking-widest text-primary">{branches.length} sucursal(es) en el alcance</Badge></div></div>
    ...
    <div className="flex min-w-0 gap-2 overflow-x-auto rounded-2xl border border-border/60 bg-muted/30 p-1.5" role="tablist" aria-label="Módulos operativos Manager">{visibleModules.map((item) => <button key={item.id} type="button" role="tab" aria-selected={module === item.id} onClick={() => changeModule(item.id)} className={cn('flex flex-none items-center gap-2 rounded-xl px-3 py-2 text-[11px] font-black uppercase tracking-wide text-muted-foreground transition-colors hover:bg-card hover:text-foreground', module === item.id && 'bg-primary text-primary-foreground shadow-sm')}><item.icon className="size-3.5" />{item.label}</button>)}</div>
    <div className="flex min-w-0 gap-2 overflow-x-auto rounded-2xl border border-border/60 bg-card p-1.5" role="tablist" aria-label={`Vistas de ${definition.label}`}>{definition.views.map((item) => <button key={item.id} type="button" role="tab" aria-selected={activeView === item.id} onClick={() => changeView(item.id)} className={cn('flex-none rounded-xl px-3 py-2 text-[11px] font-black uppercase tracking-wide text-muted-foreground hover:bg-muted hover:text-foreground', activeView === item.id && 'bg-primary text-primary-foreground')}>{item.label}</button>)}</div>
    ...
    {query.isLoading ? <LoadingState /> : activeView === 'overview' ? <Overview metrics={overviewEntries} branchCount={Number(metrics.branchCount || branches.length)} /> : <>{isCompact ? <Cards rows={rows} showBranch={showBranch} onDetails={setSelectedRow} /> : <DataTable rows={rows} showBranch={showBranch} onDetails={setSelectedRow} />}{rows.length > 0 && <Pagination page={currentPage} totalPages={totalPages} total={response?.meta.total || 0} pageSize={pageSize} setPage={setPage} setPageSize={(next) => { setPageSize(next); setPage(1); }} />}</>}
    {selectedRow && <DetailCard row={selectedRow} onClose={() => setSelectedRow(null)} />}
  </div>;
}
...
function DataTable({ rows, showBranch, onDetails }: { rows: any[]; showBranch: boolean; onDetails: (row: any) => void }) { return <Card className="overflow-hidden rounded-3xl border-border/60 shadow-sm"><CardContent className="p-0">{rows.length ? <div className="sales-responsive-table overflow-x-auto"><Table className="min-w-[900px]"><TableHeader><TableRow><TableHead>Registro</TableHead><TableHead>Fecha</TableHead><TableHead>Estado</TableHead><TableHead>Detalle</TableHead><TableHead className="text-right">Valor</TableHead>{showBranch && <TableHead>Sucursal</TableHead>}<TableHead className="text-right">Acción</TableHead></TableRow></TableHeader><TableBody>{rows.map((row) => <TableRow key={row.id}><TableCell><p className="font-semibold">{formatValue(primaryOf(row))}</p><p className="text-xs text-muted-foreground">{row.id?.slice?.(0, 8) || ''}</p></TableCell><TableCell className="whitespace-nowrap">{formatDate(dateOf(row))}</TableCell><TableCell><Badge variant="outline" className="capitalize">{formatStatus(row.status)}</Badge></TableCell><TableCell className="max-w-72 break-words">{formatValue(detailOf(row))}</TableCell><TableCell className="text-right font-semibold">{formatValue(amountOf(row))}</TableCell>{showBranch && <TableCell className="whitespace-nowrap font-semibold text-primary">{row.branchName || 'Sucursal'}</TableCell>}<TableCell className="text-right"><Button type="button" variant="outline" size="sm" className="rounded-xl" onClick={() => onDetails(row)}><Eye className="mr-1.5 size-3.5" />Detalle</Button></TableCell></TableRow>)}</TableBody></Table></div> : <EmptyState />}</CardContent></Card>; }

function Cards({ rows, showBranch, onDetails }: { rows: any[]; showBranch: boolean; onDetails: (row: any) => void }) { return rows.length ? <div className="grid grid-cols-1 gap-3 md:grid-cols-2">{rows.map((row) => <Card key={row.id} className="rounded-2xl border-border/60 shadow-sm"><CardContent className="space-y-3 p-5"><div className="flex items-start justify-between gap-3"><div className="min-w-0"><p className="truncate font-black">{formatValue(primaryOf(row))}</p><p className="text-xs text-muted-foreground">{formatValue(detailOf(row))}</p></div><Badge variant="outline" className="shrink-0 capitalize">{formatStatus(row.status)}</Badge></div><div className="grid grid-cols-2 gap-3 text-sm"><div><p className="text-xs text-muted-foreground">Fecha</p><p className="font-semibold">{formatDate(dateOf(row))}</p></div><div><p className="text-xs text-muted-foreground">Valor</p><p className="font-black">{formatValue(amountOf(row))}</p></div></div>{showBranch && <p className="border-t border-border/60 pt-2 text-xs font-semibold text-primary">{row.branchName || 'Sucursal'}</p>}<Button type="button" variant="outline" className="w-full rounded-xl" onClick={() => onDetails(row)}><Eye className="mr-2 size-4" />Ver detalle</Button></CardContent></Card>)}</div> : <EmptyState />; }

function DetailCard({ row, onClose }: { row: any; onClose: () => void }) { const fields = Object.entries(row).filter(([key, value]) => !['id', 'clientTenantId', 'tenantId'].includes(key) && value !== null && value !== undefined && typeof value !== 'function'); return <Card className="rounded-3xl border-primary/25 bg-primary/5"><CardHeader className="flex flex-row items-start justify-between gap-3"><div><CardTitle className="text-lg font-black uppercase italic tracking-tight">Detalle del registro</CardTitle><p className="mt-1 text-sm text-muted-foreground">{formatValue(primaryOf(row))} · {row.branchName || 'Sucursal'}</p></div><Button type="button" variant="ghost" size="icon" aria-label="Cerrar detalle" onClick={onClose}><X className="size-4" /></Button></CardHeader><CardContent className="grid grid-cols-1 gap-x-6 gap-y-3 text-sm sm:grid-cols-2 lg:grid-cols-3">{fields.map(([key, value]) => <div key={key} className="min-w-0"><p className="text-[10px] font-black uppercase tracking-widest text-muted-foreground">{key.replaceAll(/([A-Z])/g, ' $1')}</p><p className="break-words font-semibold">{formatValue(value)}</p></div>)}</CardContent></Card>; }
