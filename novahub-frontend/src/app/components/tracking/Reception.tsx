...
export function Reception() {
  ...
  const [form, setForm] = useState({ supplierId: '', supplierName: '', warehouseId: '', warehouseValue: '', agencyId: '', agencyName: '', customerId: '', customerName: '' });
  ...
  useEffect(() => {
    (async () => {
      try {
        const [context, supplierResponse, customerResponse] = await Promise.all([
  ...
  const canSave = Boolean(
    form.agencyName && rows.length > 0 && rows.every((row) => row.shipmentModeCode && row.trackingCode?.trim().length >= 4 && Number(row.physicalWeight) > 0),
  );
  ...
  const reset = useCallback(() => {
    setRows([emptyRow(0)]);
    setReference('');
    setForm({ supplierId: '', supplierName: '', warehouseId: '', warehouseValue: '', agencyId: '', agencyName: '', customerId: '', customerName: '' });
  }, []);
  ...
  const saveReception = useCallback(async () => {
    ...
    try {
      ...
      const strategy = warehouse?.strategy || 'NONE';
      await logisticsService.addBatchPackages(batch.id, rows.map((row) => ({
        ...
        warehouseValue: strategy === 'TRACKING_LAST_N' ? undefined : form.warehouseValue.trim() || undefined,
        agency: form.agencyName ? { id: form.agencyId || undefined, name: form.agencyName } : undefined,
      ...
      <Card className="rounded-2xl border-border/60 p-5 shadow-sm" data-tour="log-reception-wizard">
        <div className="flex flex-wrap items-center gap-3">
          ...
          <label className="cursor-pointer">
            <input type="file" accept="application/pdf" multiple className="hidden" onChange={(event) => { void onPickPdfs(Array.from(event.target.files || [])); event.target.value = ''; }} />
            <Button type="button" variant="outline" className="rounded-xl text-xs" disabled={importing}><FileUp className="size-4" /> {importing ? 'Procesando PDF…' : 'Cargar PDF AWBOX / OGLOBAL'}</Button>
          </label>
        </div>
        ...
        <div className="mt-5 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
          ...
          <div>
            ...
            <Input list="reception-agencies" value={form.agencyName} onChange={(event) => { const customer = customers.find((item) => item.name === event.target.value); setForm((current) => ({ ...current, agencyName: event.target.value, agencyId: customer?.id || '' })); }} placeholder="Buscar cliente/agencia…" className="rounded-xl" />
          ...
          <div>
            ...
            <Input list="reception-customers" value={form.customerName} onChange={(event) => { const customer = customers.find((item) => item.name === event.target.value); setForm((current) => ({ ...current, customerName: event.target.value, customerId: customer?.id || '' })); }} placeholder="Buscar cliente…" className="rounded-xl" />
          ...
          <div>
            ...
            <select value={form.warehouseId} onChange={(event) => setForm((current) => ({ ...current, warehouseId: event.target.value, warehouseValue: '' }))} className="w-full rounded-xl border border-input bg-background px-3 py-2 text-sm">
        ...
        {warehouse && warehouse.strategy !== 'TRACKING_LAST_N' && <div className="mt-3 max-w-sm"><label className="mb-1 block text-[10px] font-black uppercase tracking-widest text-muted-foreground">Número de bodega</label><Input value={form.warehouseValue} onChange={(event) => setForm((current) => ({ ...current, warehouseValue: event.target.value }))} placeholder="Se configura una sola vez para la recepción" className="rounded-xl font-mono" /></div>}
        {warehouse?.strategy === 'TRACKING_LAST_N' && <p className="mt-3 rounded-xl bg-primary/5 px-3 py-2 text-xs text-muted-foreground">El número de bodega se completa automáticamente desde la configuración usando los últimos {warehouse.trackingLastN} caracteres de cada tracking.</p>}

        <div className="mt-5 overflow-x-auto rounded-xl border border-border/60">
          <div className="flex items-center justify-between border-b border-border/50 bg-muted/20 px-3 py-2"><div><p className="text-xs font-black uppercase tracking-widest">Paquetes de esta referencia</p><p className="text-[11px] text-muted-foreground">Puedes mezclar aéreo y marítimo; cada fila conserva su propio tipo.</p></div><Button type="button" variant="outline" className="rounded-xl text-xs" onClick={() => setRows((previous) => [...previous, emptyRow(previous.length)])}><Plus className="size-4" /> Agregar paquete</Button></div>
          <table className="w-full min-w-[78rem] text-sm"><thead className="bg-muted/40 text-left"><tr>
            ...
            {rows.map((row, index) => <tr key={row.id} className="border-t border-border/40 align-middle">
