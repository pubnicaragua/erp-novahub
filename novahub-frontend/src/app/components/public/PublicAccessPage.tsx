...
function AdditionalChargesSummary({ document, light = false }: { document: any; light?: boolean }) {
  ...
  return <div className={`space-y-1 border-t pt-2 ${light ? 'border-slate-200' : 'border-slate-700'}`}>
    {charges.map((charge, index) => <p key={`${charge.label}-${index}`} className={light ? 'text-slate-600' : 'text-slate-300'}>{charge.label}: <span className="float-right">{formatAmount(charge.amount, document.currency)}</span></p>)}
  </div>;
}
