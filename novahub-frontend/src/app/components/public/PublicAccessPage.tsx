...
function PublicItemDescription({ item }: { item: any }) {
  const productCode = item?.productCode || item?.product?.code || item?.variant?.product?.code;
  const variantSku = item?.variantSku || item?.variant?.sku;
  const variantName = item?.variantName || item?.variant?.name;
  const attributes = formatPdfVariantAttributes(item?.variantAttributes ?? item?.variant?.attributes);
  return <div>
    <p>{item?.description || '—'}</p>
    {productCode && <p className="mt-1 font-mono text-xs text-emerald-600">Código: {productCode}</p>}
    {variantSku && <p className="font-mono text-xs text-slate-500">SKU variante: {variantSku}</p>}
    {variantName && <p className="text-xs text-slate-500">Nombre variante: {variantName}</p>}
    {attributes && <p className="text-xs text-slate-500">Atributos: {attributes}</p>}
  </div>;
}
