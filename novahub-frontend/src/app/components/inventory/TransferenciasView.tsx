...
function TransferItemEditor({
  ...
  return (
    <div className="grid min-w-0 grid-cols-[minmax(0,1fr)_auto] items-center gap-2 rounded-lg border border-border/40 bg-background/50 p-2">
      <div className="col-span-2 min-w-0" data-testid="inventory-transfer-product">
        <Combobox
      ...
      <div className="col-span-2 flex min-w-0 items-center gap-2">
        ...
        <Button type="button" variant="ghost" size="icon" className="ml-auto size-8 shrink-0 text-destructive hover:bg-destructive/10" onClick={onRemove} disabled={saving} aria-label="Quitar producto">
          <X className="size-4" />
        </Button>
      </div>
    </div>
...
export function TransferenciasView({ transfers, warehouses, products, series = [], onRefresh, pagination, onSearchChange, branches = [], selectedBranchId, onGoToConfig }: TransferenciasViewProps) {
  ...
  const addItem = () => {
    setNewTransfer((prev) => ({ ...prev, items: [...prev.items, { key: nextItemKey(), productId: '', variantId: '', quantity: 1, serials: [] }] }));
  };
  ...
  const updateItem = (key: string, patch: Partial<TransferItemDraft>) => {
    if (patch.quantity !== undefined) {
      ...
      const available = item ? getAvailableOriginStock(item.productId, item.variantId) : null;
  ...
  const handleItemProductChange = (key: string, productId: string) => {
    ...
    const initialVariantId = product?.variants?.length === 1 ? product.variants[0].id : '';
    const available = getAvailableOriginStock(productId, initialVariantId);
    ...
    updateItem(key, { productId, variantId: initialVariantId, serials: [] });
  };
  ...
  const getAvailableOriginStock = (productId: string, variantId?: string): number | null => {
    ...
    const levels = Array.isArray(product?.stockLevels)
      ? product.stockLevels.filter((level: any) => String(level?.warehouseId || level?.warehouse?.id || '') === String(newTransfer.fromId)
        && (!variantId || String(level?.variantId || level?.variant?.id || '') === String(variantId)))
      : [];
  ...
  const renderStockDelta = (transfer: any) => {
    ...
    return (
      <div className="flex flex-wrap items-center justify-center gap-1.5">
        <Badge variant="outline" className="bg-destructive/10 text-destructive border-destructive/20 text-[9px] font-black">-{units}</Badge>
        <span className="max-w-28 truncate text-[10px] text-muted-foreground">{transfer.from?.name || '—'}</span>
        <ArrowRight className="size-3 shrink-0 text-muted-foreground" />
        <Badge variant="outline" className="bg-success/10 text-success border-success/20 text-[9px] font-black">+{units}</Badge>
  ...
  const handleCreateTransfer = async () => {
    ...
    for (const item of draftItems) {
      ...
      const variantId = item.variantId || (product?.variants?.length === 1 ? product.variants[0]?.id : '');
      if (!product || !variantId) {
        toast.error(`Selecciona la variante de "${product?.name || item.productId}" antes de transferir`);
        return;
      }
      ...
      const available = getAvailableOriginStock(item.productId, variantId);
  ...
  return (
    <Card className="p-4 border bg-card rounded-xl">
        ...
        {(canCreateTransfer || canEditTransferConfig) && (
          <div className="flex w-full flex-col gap-2 md:w-auto md:flex-row md:flex-wrap md:items-center md:justify-end">
            ...
            {canCreateTransfer && <Button 
              ...
              data-testid="inventory-transfer-new"
            >
      ...
      {isCreating && newTransfer.fromId && newTransfer.toId && (
        <div className={`mb-4 flex items-start gap-2 rounded-xl border p-3 text-xs ${accountingPreflightLoading || accountingPreflight?.ready ? 'border-success/20 bg-success/5 text-success' : 'border-warning/25 bg-warning/5 text-warning'}`}>
          ...
          <TableBody>
              ...
              <TableRow className="bg-info/5">
                ...
                <TableCell data-tour="inventory-transfer-form-data">
                  <Select value={newTransfer.fromId} onValueChange={handleFromWarehouseChange}>
                    <SelectTrigger className="h-8 text-xs" data-testid="inventory-transfer-source"><SelectValue placeholder="Origen" /></SelectTrigger>
                    <SelectContent>
                ...
                <TableCell>
                  <Select value={newTransfer.toId} onValueChange={(v) => setNewTransfer({...newTransfer, toId: v})}>
                    <SelectTrigger className="h-8 text-xs" data-testid="inventory-transfer-destination"><SelectValue placeholder="Destino" /></SelectTrigger>
                    <SelectContent>
                ...
                <TableCell>
                  <div className="flex min-w-72 flex-col gap-2">
                    ...
                    <Button
                      ...
                      data-testid="inventory-transfer-add-item"
                    >
            ...
            {filteredTransfers.length === 0 && !isCreating ? (
              ...
              filteredTransfers.map((trf: any) => (
                  <TableRow key={trf.id} aria-busy={String(openingId) === String(trf.id) || undefined} data-detail-opening={String(openingId) === String(trf.id) ? 'true' : undefined} className="group cursor-pointer hover:bg-muted/30" onClick={() => openTransfer(trf)}>
                    <TableCell className="font-mono text-xs"><span className="inline-flex items-center gap-2">{trf.number}{String(openingId) === String(trf.id) && <span role="status" className="inline-flex items-center gap-1 font-sans text-[9px] font-black uppercase tracking-wider text-primary"><Loader2 className="size-3 animate-spin" /> Abriendo…</span>}</span></TableCell>
