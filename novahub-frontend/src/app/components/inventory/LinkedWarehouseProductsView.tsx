          ...
          {warehouses.map((warehouse) => (
            <TabsContent key={warehouse.id} value={warehouse.id} className="m-0 space-y-4">
              <div className="grid grid-cols-2 gap-3 sm:grid-cols-3">
                ...
                <Card className="rounded-2xl border-border/60"><CardContent className="p-4"><p className="text-[10px] font-black uppercase tracking-widest text-muted-foreground">Disponibles</p><p className="mt-1 text-2xl font-black tabular-nums text-success">{numberFormat.format(availableUnits)}</p></CardContent></Card>
              </div>
