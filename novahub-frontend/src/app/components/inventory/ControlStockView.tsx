...
const REASON_OPTIONS = [
  ...
  { value: 'MIXED', label: 'Ajuste mixto' },
];
...
function roundReferenceCost(value: unknown): number {
...
function formatReferenceCost(value: unknown): string {
  return roundReferenceCost(value).toFixed(2);
}
    ...
    try {
      await inventoryService.createAdjustment({
        ...
        items: [{
          ...
          unitCost: roundReferenceCost(newAdjustment.unitCost),
      ...
      await Promise.all(
        validAllocations.map((item) =>
          inventoryService.createMovement({
            ...
            unitCost: roundReferenceCost(newReception.unitCost),
                ...
                {canViewInventoryCost && <TableCell className="text-right">
                  <div className="flex items-center gap-1">
                    <Input
                      ...
                      onChange={(e) => setNewAdjustment({ ...newAdjustment, unitCost: roundReferenceCost(e.target.value) })}
                    ...
                    {canViewInventoryCost && <TableCell className="text-right text-xs">
                      {adj.items && adj.items[0] && (
                        <div className="flex flex-col">
                          <span>{adj.items[0].currency} {formatReferenceCost(adj.items[0].unitCost)}</span>
                        </div>
                      )}
                    </TableCell>}
                    <TableCell>
              ...
              {canViewInventoryCost && <div>
                ...
                <div className="flex items-center gap-2">
                  <Input
                    ...
                    onChange={(e) => setNewReception({ ...newReception, unitCost: roundReferenceCost(e.target.value) })}
