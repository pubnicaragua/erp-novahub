...
export function PdfDownloadButton({ onDownload, className, disabled = false, size = 'sm' }: PdfDownloadButtonProps) {
  const standardOptions = PDF_DOWNLOAD_OPTIONS.filter((option) => option.group === 'standard');
  const rollOptions = PDF_DOWNLOAD_OPTIONS.filter((option) => option.group === 'roll');
  ...
  return (
    <DropdownMenu>
      ...
      <DropdownMenuContent align="end" className="w-64 rounded-2xl p-1.5">
        <DropdownMenuItem onClick={() => onDownload('configured')} className="gap-2 rounded-xl py-2.5 [&_svg]:text-foreground/80 data-[highlighted]:[&_svg]:!text-primary-foreground">
          <Download className="size-4" />
          <span className="min-w-0 flex-1">
            <span className="block font-bold">PDF normal</span>
            <span className="block text-[10px] text-popover-foreground/75">Diseño asignado o global</span>
          </span>
        </DropdownMenuItem>
        <DropdownMenuSeparator />
