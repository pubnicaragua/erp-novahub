...
function QuoteEditor({ draft, setDraft, editingId, groups, preview, onSave, onClose }: { draft: DraftQuote; setDraft: Dispatch<SetStateAction<DraftQuote | null>>; editingId: string | null; groups: any[]; preview: { subtotal: number; optional: number; tax: number; total: number }; onSave: () => Promise<void>; onClose: () => void }) {
  ...
  return <Card className="rounded-3xl border-emerald-500/30 bg-emerald-500/[0.03] shadow-sm"><CardHeader className="flex-row items-center justify-between p-5 pb-3"><div><CardTitle className="text-lg font-black uppercase">{editingId ? 'Editar cotización' : 'Nueva cotización'}</CardTitle><p className="mt-1 text-sm text-muted-foreground">Los conceptos pueden ser anuales, mensuales o de pago único.</p></div><Button variant="ghost" size="icon" onClick={onClose}><X className="size-5" /></Button></CardHeader><CardContent className="space-y-5 p-5">
    ...
    <div className="overflow-x-auto rounded-2xl border border-border/70"><table className="w-full min-w-[980px] text-sm"><thead className="bg-muted/30 text-left text-[10px] font-black uppercase tracking-widest text-muted-foreground"><tr><th className="px-3 py-3">Sección</th><th className="px-3 py-3">Concepto</th><th className="px-3 py-3">Periodicidad</th><th className="px-3 py-3">Detalle</th><th className="px-3 py-3">Cant.</th><th className="px-3 py-3">Precio</th><th className="px-3 py-3">Opcional</th><th /></tr></thead><tbody>{draft.items.map((item, index) => <tr key={index} className="border-t border-border/50"><td className="p-2"><SmallInput value={item.section} onChange={(value) => updateItem(index, 'section', value)} /></td><td className="p-2"><SmallInput value={item.description} onChange={(value) => updateItem(index, 'description', value)} /></td><td className="p-2"><select value={item.periodicity || ''} onChange={(event) => updateItem(index, 'periodicity', event.target.value)} className="h-9 w-full min-w-0 rounded-lg border border-border bg-background px-2 text-xs outline-none focus:border-primary"><option value="Mensual">Mensual</option><option value="Anual">Anual</option><option value="Pago único">Pago único</option></select></td><td className="p-2"><SmallInput value={item.detail || ''} onChange={(value) => updateItem(index, 'detail', value)} /></td><td className="p-2"><SmallInput type="number" value={item.quantity} onChange={(value) => updateItem(index, 'quantity', numberValue(value))} /></td><td className="p-2"><SmallInput type="number" value={item.unitPrice} onChange={(value) => updateItem(index, 'unitPrice', numberValue(value))} /></td><td className="p-2 text-center"><input type="checkbox" checked={item.isOptional} onChange={(event) => updateItem(index, 'isOptional', event.target.checked)} /></td><td className="p-2"><Button variant="ghost" size="icon" onClick={() => updateDraft((current) => ({ ...current, items: current.items.filter((_, itemIndex) => itemIndex !== index) }))}><Trash2 className="size-4 text-red-500" /></Button></td></tr>)}</tbody></table></div>
    <div className="flex flex-wrap items-center justify-between gap-3"><Button variant="outline" className="rounded-xl" onClick={() => updateDraft((current) => ({ ...current, items: [...current.items, { section: '2. MÓDULOS Y SERVICIOS ADICIONALES', description: '', periodicity: 'Mensual', detail: '', quantity: 1, unitPrice: 0, isOptional: true }] }))}><Plus className="mr-2 size-4" /> Agregar concepto</Button><div className="grid min-w-[270px] gap-1 text-right text-sm"><p className="text-muted-foreground">Base: <strong>{money(preview.subtotal, draft.currency)}</strong> · Opcionales: <strong>{money(preview.optional, draft.currency)}</strong></p><p className="text-xl font-black text-emerald-600">Total: {money(preview.total, draft.currency)}</p></div></div>
