...
const defaultItems: DraftItem[] = [
  ...
  { section: '2. MÓDULOS Y SERVICIOS ADICIONALES', description: 'Dominio propio', periodicity: 'Anual', detail: 'Incluye 5 correos corporativos', quantity: 1, unitPrice: 100, isOptional: false },
  { section: '2. MÓDULOS Y SERVICIOS ADICIONALES', description: 'Módulo contable', periodicity: 'Mensual', detail: '', quantity: 1, unitPrice: 100, isOptional: false },
  { section: '2. MÓDULOS Y SERVICIOS ADICIONALES', description: 'Módulo de RR. HH.', periodicity: 'Mensual', detail: '', quantity: 1, unitPrice: 75, isOptional: false },
  { section: '2. MÓDULOS Y SERVICIOS ADICIONALES', description: 'E-commerce', periodicity: 'Anual', detail: '', quantity: 1, unitPrice: 600, isOptional: false },
  { section: '2. MÓDULOS Y SERVICIOS ADICIONALES', description: 'Landing page', periodicity: 'Anual', detail: '', quantity: 1, unitPrice: 300, isOptional: false },
  { section: '2. MÓDULOS Y SERVICIOS ADICIONALES', description: 'Píxel de Meta', periodicity: 'Pago único', detail: '', quantity: 1, unitPrice: 100, isOptional: false },
  { section: '2. MÓDULOS Y SERVICIOS ADICIONALES', description: 'Nova Suite', periodicity: 'Mensual', detail: '', quantity: 1, unitPrice: 49.99, isOptional: false },
  { section: '2. MÓDULOS Y SERVICIOS ADICIONALES', description: 'Conexión de Nova Suite', periodicity: 'Pago único', detail: '', quantity: 1, unitPrice: 100, isOptional: false },
];
...
const emptyDraft = (): DraftQuote => ({
  ...
  enterpriseGroupId: '', clientTenantId: '', discountAmount: 0, taxRate: 0, notes: '',
    ...
    <CardContent className="space-y-4 p-3 sm:p-6">
      ...
      <div className="overflow-x-auto pb-2"><div className="mx-auto min-w-[760px] max-w-[980px] overflow-hidden rounded-[5px] bg-white shadow-[0_20px_70px_rgba(15,23,42,0.16)] ring-1 ring-slate-900/10">
        ...
        <div className="space-y-4 p-6 sm:p-8"><div className="grid gap-4 rounded-2xl bg-[#effaf4] p-5 sm:grid-cols-[1fr_auto] sm:items-center"><div><p className="text-[9px] font-black uppercase tracking-[0.16em] text-slate-500">Preparada para</p><DocumentInput value={draft.prospectCompany} placeholder="Empresa prospecto *" onChange={(value) => field('prospectCompany', value)} className="mt-1 text-lg font-black text-slate-950" /><div className="mt-1 flex flex-wrap gap-x-2 gap-y-1 text-[10px] text-slate-500"><DocumentInput value={draft.prospectName} placeholder="Contacto *" onChange={(value) => field('prospectName', value)} /><span>·</span><DocumentInput value={draft.prospectEmail} placeholder="correo" onChange={(value) => field('prospectEmail', value)} /><span>·</span><DocumentInput value={draft.prospectPhone} placeholder="teléfono" onChange={(value) => field('prospectPhone', value)} /><span>·</span><DocumentInput value={draft.country} placeholder="país" onChange={(value) => field('country', value)} /></div></div><div className="sm:text-right"><p className="text-[9px] font-black uppercase tracking-[0.16em] text-slate-500">Moneda</p><p className="mt-1 text-lg font-black text-slate-950">{draft.currency}</p></div></div>
          ...
          <table className="w-full border-collapse text-[10px]"><thead><tr className="bg-[#0A0A0A] text-left text-[9px] font-black uppercase tracking-wider text-white"><th className="w-[27%] px-3 py-3">Concepto</th><th className="w-[15%] px-2 py-3 text-right">Precio unit.</th><th className="w-[15%] px-2 py-3 text-center">Cobro</th><th className="w-[28%] px-2 py-3">Detalle</th><th className="w-[15%] px-3 py-3 text-right">Importe</th></tr></thead><tbody>{draft.items.map((item, index) => <Fragment key={`quote-row-${index}`}>{(index === 0 || item.section !== draft.items[index - 1]?.section) && <tr><td colSpan={5} className="border-b border-emerald-900/10 bg-[#e1f5e9] px-3 py-2"><DocumentInput value={item.section} placeholder="Sección" onChange={(value) => updateItem(index, 'section', value)} className="w-full text-[10px] font-black uppercase text-slate-950" /></td></tr>}<tr className="group border-b border-slate-200"><td className="px-3 py-1.5"><div className="flex items-center gap-1"><DocumentInput value={item.description} placeholder="Concepto" onChange={(value) => updateItem(index, 'description', value)} className="flex-1 font-medium text-slate-800" /><button type="button" aria-label="Eliminar concepto" title="Eliminar concepto" onClick={() => removeItem(index)} className="ml-auto shrink-0 rounded p-1 text-slate-300 opacity-0 transition hover:bg-red-50 hover:text-red-500 group-hover:opacity-100 focus:opacity-100"><Trash2 className="size-3" /></button></div></td><td className="px-2 py-1.5 text-right"><div className="flex items-center justify-end gap-1"><span className="text-[9px] text-slate-400">×</span><DocumentNumberInput value={item.quantity} onChange={(value) => updateItem(index, 'quantity', numberValue(value))} className="w-7 text-center" /><span className="text-[9px] text-slate-500">{draft.currency}</span><DocumentNumberInput value={item.unitPrice} onChange={(value) => updateItem(index, 'unitPrice', numberValue(value))} className="w-16" /></div></td><td className="px-2 py-1.5 text-center"><select value={item.periodicity} onChange={(event) => updateItem(index, 'periodicity', event.target.value)} className="h-7 rounded-md border border-transparent bg-transparent text-[10px] font-medium text-slate-600 outline-none transition hover:border-slate-300 focus:border-primary focus:bg-primary/10"><option value="Mensual">Mensual</option><option value="Anual">Anual</option><option value="Pago único">Pago único</option></select></td><td className="px-2 py-1.5"><DocumentInput value={item.detail} placeholder="—" onChange={(value) => updateItem(index, 'detail', value)} className="text-[10px] text-slate-500" /></td><td className="px-3 py-1.5 text-right font-bold text-slate-900">{money(Number(item.quantity || 0) * Number(item.unitPrice || 0), draft.currency)}</td></tr></Fragment>)}</tbody></table>
          ...
          <div className="border-t border-slate-200 pt-4"><p className="text-[9px] font-black uppercase tracking-[0.16em] text-slate-700">Términos y condiciones</p><textarea value={draft.notes} onChange={(event) => field('notes', event.target.value)} rows={2} className="mt-1 w-full resize-none rounded-md border border-transparent bg-transparent p-0 text-[10px] leading-relaxed text-slate-500 outline-none transition focus:border-primary focus:bg-primary/10" /></div>
