...
import { Plus, Search, CheckCircle2, Clock, AlertTriangle, ListTodo } from 'lucide-react';
...
export const TareasView: React.FC<TareasViewProps> = ({ data, loading, onRefresh }) => {
  ...
  const columns = [
    ...
    { key: 'dueDate', header: 'Vencimiento', width: '120px', editable: true, type: 'date' as const, render: (val: any) => val ? format(new Date(val), 'MMM dd, yyyy') : '-' },
  ...
  const handleAdd = async () => {
    try {
      await tasksService.create({ title: 'Nueva Tarea', status: 'PENDING' as any, priority: 'MEDIUM' as any });
      toast.success('Tarea creada'); onRefresh();
      ...
      <Card className="border-none bg-background/50 backdrop-blur-xl shadow-sm">
        <div className="p-4 border-b border-border/50 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-3">
            ...
            <Button onClick={handleAdd} className="bg-primary hover:bg-primary/90 text-primary-foreground font-black uppercase text-[10px] tracking-widest px-4 h-10 rounded-xl gap-2"><Plus className="size-4" /> Nueva Tarea</Button>
          </div>
        </div>
