...
export function ComisionesView() {
  ...
  const detailRows = useMemo(() => reportItems.flatMap((item) => getCommissionLines(item, baseCurrency).map((line, index) => ({ ...line, seller: item.seller, commissionId: item.id, lineIndex: index }))), [reportItems, baseCurrency]);
  ...
  const downloadCsv = async () => {
    ...
    try {
      ...
      items.forEach((item) => getCommissionLines(item, baseCurrency).forEach((line) => detailRowsForExport.push([item.seller?.name || '', formatDate(line.date), formatTime(line.date), line.code || '', Number(line.quantity || 0), Number(line.salePrice || 0), Number(line.productSubtotal || 0), Number(line.totalSale || 0), line.priceType || '', Number(line.commission || 0), line.invoiceNumber || item.invoice?.number || '', line.customer || '', line.paymentForm || '', line.paymentDetail || '', statusLabel(line.status)])));
  ...
  return (
    <div className="min-w-0 space-y-4">
      ...
      <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 xl:grid-cols-5"><SalesKpiCard title="Vendedores con ventas" value={String(summary.sellerCount || 0)} icon={Users} color="text-primary" bg="bg-primary/10" /><SalesKpiCard title="Venta base total" value={fmt(Number(summary.salesBase || 0), baseCurrency)} icon={ReceiptText} color="text-foreground" bg="bg-muted" /><SalesKpiCard title="Comisión total" value={fmt(Number(summary.commissionBase || 0), baseCurrency)} icon={BadgeDollarSign} color="text-primary" bg="bg-primary/10" /><SalesKpiCard title="Pendiente" value={fmt(Number(summary.pendingBase || 0), baseCurrency)} icon={Clock3} color="text-amber-600" bg="bg-amber-500/10" /><SalesKpiCard title="Pagada en nómina" value={fmt(Number(summary.paidBase || 0), baseCurrency)} icon={WalletCards} color="text-emerald-600" bg="bg-emerald-500/10" /></div>
