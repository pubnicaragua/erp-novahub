  ...
  const renderMobileCard = (row: EmployeeImportRow, index: number) => {
    ...
    return (
      <ImportPreviewMobileCard index={index} title={[row.firstName, row.lastName].filter(Boolean).join(' ') || row.employeeNumber} error={row._hasError ? row._errorMessage || 'Fila con errores' : undefined} warning={row._hasWarning ? row._warningMessage || 'Revisar fila' : undefined}>
        ...
        {row._hasError && (!row.departmentId || !row.positionId) && <p className="mt-3 rounded-xl border border-destructive/20 bg-destructive/5 p-3 text-xs text-muted-foreground">Corrige el catálogo o usa + para crearlo.</p>}
  ...
  return (
    <div data-import-preview-shell="true" className={`fixed inset-y-0 right-0 left-0 z-40 flex h-dvh min-h-0 flex-col overflow-hidden bg-background p-3 sm:p-6 ${isSidebarCollapsed ? 'lg:left-[72px]' : 'lg:left-[270px]'}`}>
      <div className="mx-auto flex min-h-0 w-full max-w-[1900px] flex-1 flex-col gap-4">
          ...
          <Table containerClassName="overflow-visible" containerStyle={{ width: '4230px', minWidth: '4230px', maxWidth: 'none' }} className="block w-[4230px] min-w-[4230px]">
            ...
            <TableBody style={{ display: 'block', position: 'relative', height: tableVirtualizer.getTotalSize() }}>
              {tableVirtualizer.getVirtualItems().map((virtualRow) => {
                ...
                const rowPositions = positions.filter((position: any) => !row.departmentId || position.departmentId === row.departmentId);
                return (
                  <TableRow key={virtualRow.key} ref={tableVirtualizer.measureElement} data-index={virtualRow.index} style={{ display: 'grid', gridTemplateColumns: gridTemplate, position: 'absolute', left: 0, top: 0, width: '100%', transform: `translateY(${virtualRow.start}px)` }} className={row._hasError ? 'bg-destructive/5' : row._hasWarning ? 'bg-warning/5' : ''}>
                    <TableCell className="text-center">{row._hasError ? <AlertTriangle className="mx-auto size-4 text-destructive" /> : row._hasWarning ? <AlertTriangle className="mx-auto size-4 text-warning" /> : <CheckCircle2 className="mx-auto size-4 text-success" />}</TableCell>
                    ...
                    <TableCell className="whitespace-normal">
                      <div className="space-y-2">
                        <p className={row._hasError ? 'text-xs font-medium text-destructive' : row._hasWarning ? 'text-xs font-medium text-warning' : 'text-xs text-success'}>{row._errorMessage || row._warningMessage || 'Correcto'}</p>
        ...
        <section data-import-preview-mobile-section="true" className="flex min-h-0 flex-1 flex-col overflow-hidden rounded-2xl border bg-card p-3 sm:hidden" aria-label="Registros de empleados para revisar">
      ...
      <Dialog open={result !== null} onOpenChange={(open) => { if (!open) onDone(); }}>
        <DialogContent className="max-w-lg">
          <DialogHeader data-tour="hr-employee-import-result-title"><div className="flex flex-col items-center gap-3 py-3 text-center"><div className="flex size-20 animate-in zoom-in items-center justify-center rounded-full bg-success/10 text-success duration-500"><CheckCircle2 className="size-12 animate-pulse" /></div><DialogTitle className="text-xl">Importación procesada</DialogTitle><DialogDescription>Los empleados válidos ya fueron registrados. Puedes volver a importar otro archivo cuando quieras.</DialogDescription></div><HRViewTutorial label="Cómo consultar resultado de importación" targetPrefix="hr-employee-import-result" stepKeys={['title', 'data', 'actions']} copy={{ data: { description: 'Revisa empleados creados, avisos y filas omitidas.' }, actions: { description: 'Descarga errores o continúa al listado de empleados.' } }} /></DialogHeader>
          <div className="grid grid-cols-3 gap-2 text-center" data-tour="hr-employee-import-result-data"><div className="rounded-xl border bg-muted/20 p-3"><p className="text-2xl font-black text-success">{result?.created || 0}</p><p className="text-[10px] uppercase text-muted-foreground">Creados</p></div><div className="rounded-xl border bg-muted/20 p-3"><p className="text-2xl font-black text-warning">{result?.warnings.length || 0}</p><p className="text-[10px] uppercase text-muted-foreground">Avisos</p></div><div className="rounded-xl border bg-muted/20 p-3"><p className="text-2xl font-black text-destructive">{result?.skipped || 0}</p><p className="text-[10px] uppercase text-muted-foreground">Omitidos</p></div></div>
