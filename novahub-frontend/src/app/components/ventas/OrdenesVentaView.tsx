...
const statusOptions = [
  ...
  { label: 'Cerrada', value: 'closed', color: 'bg-blue-500/10 text-blue-500' },
  { label: 'Anulada', value: 'void', color: 'bg-rose-500/10 text-rose-500' },
];
...
export function OrdenesVentaView({ data, loading, onRefresh, onGenerateInvoice }: OrdenesVentaViewProps) {
  ...
  const kpis = [
    { title: 'Órdenes Abiertas', value: data.filter(o => o.status === 'confirmed').length, icon: Package, color: 'text-orange-500', bg: 'bg-orange-500/10' },
    { title: 'Monto por Facturar', value: `$${data.filter(o => o.status === 'confirmed').reduce((acc, o) => acc + o.total, 0).toLocaleString()}`, icon: TrendingUp, color: 'text-emerald-500', bg: 'bg-emerald-500/10' },
    { title: 'Pendientes Hoy', value: '3', icon: Clock, color: 'text-blue-500', bg: 'bg-blue-500/10' },
        ...
        <EditableDataTable 
          ...
          actions={(row) => (
            <div className="flex items-center gap-1">
               ...
               <Button variant="ghost" size="icon" className="size-8 rounded-lg hover:bg-rose-500/10 hover:text-rose-500 transition-colors" onClick={() => salesOrdersService.delete(row.id).then(() => onRefresh())}><Trash2 className="size-4" /></Button>
            </div>
          )}
        />
      </div>
