...
import { cajaService, type InvoiceCashQueue } from '../../../services/caja.service';
...
const formatQueueAmount = (queue: InvoiceCashQueue) => {
  const symbol = queue.invoice.currency === 'USD' ? '$' : 'C$';
  return `${symbol} ${Number(queue.invoice.balance || 0).toLocaleString('es-NI', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`;
};
  ...
  return (
    <DropdownMenu onOpenChange={(open) => { if (open) void load(); }}>
      ...
      <DropdownMenuContent align="end" className="w-[min(22rem,calc(100vw-2rem))] rounded-xl border-border/60 p-2">
        ...
        {!error && items.length === 0 && <p className="px-2 py-4 text-xs text-muted-foreground">No hay facturas pendientes o tomadas recientemente.</p>}
        {items.length > 0 && (
                ...
                <DropdownMenuItem key={queue.id} className="items-start gap-2 rounded-lg px-3 py-2.5" onSelect={() => openQueueItem(queue.id)}>
                  ...
                  <span className="min-w-0 flex-1">
                    <span className="flex items-center justify-between gap-2">
                      <span className="truncate text-xs font-black text-foreground">{queue.invoice.number}</span>
                      <span className="shrink-0 font-mono text-[10px] font-bold text-primary">{formatQueueAmount(queue)}</span>
                    </span>
                    <span className="mt-0.5 block truncate text-[10px] text-muted-foreground">{queue.invoice.customer?.name || queue.invoice.customCustomerName || 'Cliente general'}</span>
