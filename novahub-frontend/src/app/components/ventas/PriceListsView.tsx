  ...
  return (
    <div data-import-preview-shell="true" className={`fixed inset-y-0 right-0 left-0 z-40 flex h-dvh min-h-0 flex-col overflow-hidden bg-background p-3 sm:p-6 ${isSidebarCollapsed ? 'lg:left-[72px]' : 'lg:left-[270px]'}`}>
      <div className="mx-auto flex min-h-0 w-full max-w-[1800px] flex-1 flex-col gap-4">
        ...
        <section data-import-preview-mobile-section="true" className="flex min-h-0 min-w-0 max-w-full flex-1 flex-col overflow-hidden rounded-2xl border bg-card p-3 sm:hidden" aria-label="Actualizaciones de precios para revisar">
...
export function PriceListsView({ products = [], onRefresh, isSidebarCollapsed = true }: PriceListsViewProps) {
  ...
  return <div className="space-y-5">
    ...
    <Dialog open={importOpen} onOpenChange={(open) => { if (!open && !importing) clearPriceImportFile(); setImportOpen(open); }}>
      <DialogContent className="max-h-[90vh] w-[calc(100vw-2rem)] !max-w-2xl overflow-y-auto">
        ...
        {!importFile ? <div className="flex flex-col items-center gap-3 rounded-2xl border-2 border-dashed p-8 text-center sm:p-12"><FileSpreadsheet className="size-12 text-primary" /><p className="font-bold">Carga el archivo Excel de precios</p><p className="text-xs text-muted-foreground">La tabla editable aparecerá en una vista completa después de cargarlo.</p><Button variant="outline" onClick={() => fileRef.current?.click()}><Upload className="mr-2 size-4" />Seleccionar archivo</Button><input ref={fileRef} type="file" accept=".xlsx,.xls,.csv" className="hidden" onChange={(event) => { if (event.target.files?.[0]) void readFile(event.target.files[0]); event.currentTarget.value = ''; }} /></div> : <div className="flex min-w-0 flex-wrap items-center gap-3 rounded-2xl border border-emerald-500/30 bg-emerald-500/5 p-4 sm:p-5"><CheckCircle2 className="size-8 shrink-0 text-emerald-500" /><div className="min-w-0 flex-1"><p className="font-bold">Archivo listo para previsualizar</p><p className="break-words text-sm text-muted-foreground">{importFile} · {importRows.length} filas detectadas</p></div><Button type="button" variant="ghost" size="sm" className="shrink-0 text-destructive hover:text-destructive" onClick={clearPriceImportFile} disabled={previewLoading || importing}><X className="mr-1.5 size-3.5" />Quitar archivo</Button></div>}
