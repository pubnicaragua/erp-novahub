...
export function FinanceReceivablesView() {
  ...
  const invoicesQuery = useQuery({
    ...
    queryFn: ({ signal }) => sourceDocumentsService.getLookup({ origin: 'SALES', page: 1, pageSize: 200 }, signal),
  ...
  const debtorTotals = pending.reduce((acc: Record<string, number>, inv: any) => { const name = inv.client?.name || inv.customerName || inv.customer?.name || 'Cliente'; acc[name] = (acc[name] || 0) + balanceOf(inv); return acc }, {} as Record<string, number>)
  const topDebtors = (Object.entries(debtorTotals) as [string, number][]).sort(([, a], [, b]) => b - a).slice(0, 5).map(([name, amount]) => ({ name, amount }))
  ...
  return (
    <div className="min-w-0 space-y-6">
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
        {renderMoneyKpi('Total por Cobrar', totalPending, 'text-amber-500', originalPending)}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
          ...
          <CardContent className="min-w-0 overflow-visible px-2 pb-3">
            <ResponsiveContainer width="100%" height={280}>
              <BarChart data={agingData} margin={{ top: 12, right: 20, left: 8, bottom: 14 }}>
                ...
                <Bar dataKey="amount" name="Saldo vencido" fill="url(#ageG)" radius={[6, 6, 0, 0]} maxBarSize={52} onClick={(data: any) => toast.info(`CxC vencido ${data.label}: ${fmt(data.amount)}`)} style={{ cursor: 'pointer' }} />
              </BarChart>
            </ResponsiveContainer>
          </CardContent>
        </Card>
        <Card className="min-w-0 rounded-2xl border-border/40 bg-card shadow-sm">
          <CardHeader className="pb-0 px-5 pt-4">
            <CardTitle className="text-sm font-black uppercase tracking-tight text-foreground">Principales Deudores</CardTitle>
            <p className="text-[10px] text-muted-foreground">Top 5 clientes con mayor saldo pendiente</p>
          </CardHeader>
          <CardContent className="min-w-0 overflow-visible px-2 pb-3">
            {topDebtors.length === 0 ? (
              <p className="text-xs text-muted-foreground py-12 text-center">Sin deudores</p>
            ) : (
              <ResponsiveContainer width="100%" height={280}>
                <BarChart data={topDebtors} layout="vertical" margin={{ top: 8, right: 20, left: 8, bottom: 14 }}>
                  ...
                  <Bar dataKey="amount" name="Saldo pendiente" radius={[0, 5, 5, 0]} maxBarSize={32} onClick={(data: any) => toast.info(`${data.name}: ${fmt(data.amount)}`)} style={{ cursor: 'pointer' }}>
                    {topDebtors.map((_, i) => <Cell key={i} fill={COLORS[i % COLORS.length]} />)}
                  </Bar>
                </BarChart>
              </ResponsiveContainer>
            )}
      ...
      <Card className="rounded-2xl border-border/40 bg-card shadow-sm">
        <CardHeader className="pb-2 px-5 pt-4">
          <CardTitle className="text-sm font-black uppercase tracking-tight text-foreground">Facturas Pendientes</CardTitle>
        </CardHeader>
            ...
            <div className="hidden overflow-x-auto md:block">
              <table className="w-full text-xs">
                <thead><tr className="border-b border-border text-muted-foreground">
                  ...
                  <th className="text-left py-2 font-bold uppercase tracking-wider">Emisión</th>
                  <th className="text-left py-2 font-bold uppercase tracking-wider">Vencimiento</th>
                  <th className="text-right py-2 font-bold uppercase tracking-wider">Monto</th>
                  ...
                  {pending.map((inv: any) => {
                    ...
                    return (
                      <tr key={inv.id} className="border-b border-border/20">
                        <td className="py-2 font-medium text-foreground">{inv.client?.name || inv.customerName || inv.customer?.name || '—'}</td>
