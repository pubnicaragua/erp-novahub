...
function buildGuides(videos: any[]): ChatGuide[] {
  ...
  return videos.filter((video) => video && video.title).map((video) => ({ title: String(video.title), module: video.module ? String(video.module) : 'General', description: video.description ? String(video.description) : '' }));
}
...
export function FloatingChat() {
  ...
  const sendMessage = useCallback(async (raw: string) => {
    ...
    try {
      await loadGuides();
      const response = await aiService.chat(content, { conversationId, dashboardContext: contextForMessage, guides: guidesRef.current.length ? guidesRef.current : undefined }, controller.signal);
  ...
  useEffect(() => { if (open) { void loadGuides(); window.setTimeout(() => inputRef.current?.focus(), 120); } }, [loadGuides, open]);
  ...
  return <>
    ...
    <AnimatePresence>{open && <motion.div initial={{ opacity: 0, y: 24, scale: 0.96 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: 24, scale: 0.96 }} transition={{ type: 'spring', stiffness: 300, damping: 28 }} className="fixed right-4 bottom-24 z-50 flex h-[70vh] max-h-[640px] w-[min(400px,calc(100vw-2rem))] flex-col overflow-hidden rounded-3xl border border-border/60 bg-background shadow-2xl shadow-black/40">
      <div className="relative shrink-0 overflow-hidden bg-gradient-to-br from-primary via-primary to-emerald-600 p-4 text-primary-foreground"><div className="absolute inset-0 bg-[radial-gradient(circle_at_top_right,rgba(255,255,255,0.15),transparent_55%)]" /><div className="relative flex items-center gap-3"><div className="flex size-10 items-center justify-center rounded-xl bg-white/15 backdrop-blur-md ring-1 ring-white/25"><Bot className="size-5" /></div><div className="min-w-0 flex-1"><h3 className="truncate text-sm font-black uppercase tracking-wider">Nova AI</h3><p className="truncate text-[11px] font-medium text-primary-foreground/75">Asistente empresarial con tus datos</p></div><button type="button" onClick={() => setOpen(false)} className="flex size-8 items-center justify-center rounded-lg bg-white/10 text-primary-foreground/90 transition-colors hover:bg-white/25" aria-label="Cerrar chat"><X className="size-4" /></button></div></div>
