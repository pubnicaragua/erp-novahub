...
import { BellRing, Check, CheckCircle2, Clock3, Loader2, MessageCircle, Pencil, Plus, Save, Send, Settings2, Trash2, Users } from 'lucide-react';
...
export function NovaPulseView({ canEdit, canSend }: { canEdit: boolean; canSend: boolean }) {
  ...
  const checkIntegration = async () => {
    ...
    try {
      const result = await novaPulseService.checkIntegration();
      toast.success(`Integración lista: ${result.templateName} · ${result.templateLanguage} · bandeja ${result.inboxId}.`);
    } catch (error) { toast.error(getApiErrorMessage(error, 'La integración de Chatwoot no pasó la validación.')); }
    finally { setCheckingIntegration(false); }
  };
  ...
  return <div className="space-y-6">
    ...
    <div className="flex flex-col gap-3 rounded-2xl border border-primary/20 bg-primary/[0.05] p-4 sm:flex-row sm:items-center sm:justify-between"><div><p className="font-bold">Listo para reportarte</p><p className="text-sm text-muted-foreground">El resumen se enviará desde el número corporativo de NovaHub.</p></div><div className="flex flex-wrap gap-2"><Button variant="outline" onClick={() => void checkIntegration()} disabled={checkingIntegration}><CheckCircle2 className="size-4" />{checkingIntegration ? 'Validando…' : 'Validar integración'}</Button><Button variant="outline" onClick={() => { setTestRecipientId(activeRecipients[0]?.id || ''); setTestDialog(true); }} disabled={!canSend || !activeRecipients.length}><Send className="size-4" />Enviar prueba</Button><Button onClick={() => setConfirmSend(true)} disabled={!canSend || !activeRecipients.length}><MessageCircle className="size-4" />Enviar resumen ahora</Button><Button variant="outline" onClick={() => void save()} disabled={!canEdit || saving}>{saving ? <Loader2 className="size-4 animate-spin" /> : <Save className="size-4" />}Guardar cambios</Button></div></div>

    <Card className="border-border/50 shadow-sm"><CardHeader><CardTitle className="flex items-center gap-2 text-lg"><BellRing className="size-5 text-primary" />Últimos envíos</CardTitle><CardDescription>Historial de envíos y errores por destinatario.</CardDescription></CardHeader><CardContent>{config.deliveries.length ? <div className="divide-y divide-border/50">{config.deliveries.map((delivery) => { const status = deliveryStatus(delivery.status); return <div key={delivery.id} className="flex flex-col gap-2 py-3 sm:flex-row sm:items-center sm:justify-between"><div><p className="font-semibold">{delivery.recipient?.name || maskPhone(delivery.phoneNumber)}</p><p className="text-xs text-muted-foreground">{new Date(delivery.createdAt).toLocaleString('es-NI')} · {delivery.type === 'TEST' ? 'Prueba' : delivery.type === 'SCHEDULED' ? 'Programado' : 'Manual'}</p></div><div className="flex items-center gap-2"><Badge variant="outline" className={status.className}>{status.label}</Badge>{delivery.errorCode && <span className="rounded bg-rose-500/10 px-1.5 py-0.5 text-[10px] font-bold text-rose-600">{delivery.errorCode}</span>}{delivery.errorMessage && <span className="max-w-sm truncate text-xs text-rose-600">{delivery.errorMessage}</span>}</div></div>; })}</div> : <p className="py-6 text-sm text-muted-foreground">Aún no hay envíos registrados.</p>}</CardContent></Card>
