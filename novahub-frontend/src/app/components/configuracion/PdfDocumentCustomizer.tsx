...
import {
  Barcode, ChevronDown, ChevronLeft,
  ChevronRight, FileCog, FileText, Folder, FolderPlus, ImagePlus, LayoutTemplate, Loader2, Maximize2, Palette, PanelBottom, PanelTop, Pencil, Plus, QrCode, RotateCcw, Save, Trash2, Upload, WandSparkles, ZoomIn, ZoomOut
} from 'lucide-react';
...
import { canEditPdfTemplate, getPdfTemplateTarget, isPdfTemplateFixed, normalizePdfTemplateKey, PDF_TEMPLATE_MODULES, PDF_TEMPLATE_TARGETS, type PdfTemplateTarget } from '../../services/pdf-document-catalog';
import { createDefaultTemplateDefinition, createSystemDefaultPdfSettings, definitionFromExtractedPdf, ensureDashboardChartNodes, formatPdfPageNumber, normalizePdfCompanySettings, normalizePdfPaperSettings, sanitizeTemplateDefinition, type PdfTemplateDefinition, type PdfTemplateNode } from '../../services/pdf-template-definition';
...
type PdfSettings = {
  paperSize: 'LETTER' | 'A4' | 'OFICIO' | 'LEGAL' | 'LABEL' | 'ROLL-80'; orientation: 'portrait' | 'landscape';
...
function PaperPreview({ settings, documentType, companyName, logo }: { settings: PdfSettings; documentType: PdfDocumentType; companyName: string; logo?: string | null }) {
  ...
  return <div className={cn('mx-auto w-full max-w-[520px] overflow-hidden rounded-md border bg-white text-slate-700 shadow-2xl', pageClass)} style={{ aspectRatio: getPdfPreviewAspectRatio(settings), fontFamily: settings.fontFamily, fontSize: `${settings.fontSize}px` }}>
    ...
    <div className="h-full p-[5%]" style={{ paddingLeft: `${Math.max(3, settings.margins / 2)}%`, paddingRight: `${Math.max(3, settings.margins / 2)}%` }}>
      ...
      <div className="mt-5 flex items-end justify-between border-t pt-2 text-[.7em] opacity-60" style={{ borderColor: settings.lineColor }}><span>{settings.footerText}</span>{settings.showPageNumber && <span>{formatPdfPageNumber(settings.pageNumberFormat, settings.pageNumberCustom, 1, 1)}</span>}</div>
...
export function PdfDocumentCustomizer({ tenantId, branchName = '', companyName = '', corporateColor = '#10b981', logo, canEdit = true, canCreate = true, canDelete = true }: { tenantId?: string; branchName?: string; companyName?: string; corporateColor?: string; logo?: string | null; canEdit?: boolean; canCreate?: boolean; canDelete?: boolean }) {
  const documentViewStorageKey = `novahub.pdf-customizer.view.${tenantId || 'current'}.${branchName || 'default'}`;
  ...
  const [documentType, setDocumentType] = useState<PdfDocumentType>(() => {
    try {
      const stored = window.localStorage.getItem(documentViewStorageKey);
      return stored && PDF_TEMPLATE_TARGETS.some(target => target.key === stored) ? stored as PdfDocumentType : 'ventas.estimate';
    } catch {
      return 'ventas.estimate';
    }
  });
  ...
  const [assignedDocuments, setAssignedDocuments] = useState<PdfDocumentType[]>(() => {
    try {
      const stored = window.localStorage.getItem(documentViewStorageKey);
      return stored && PDF_TEMPLATE_TARGETS.some(target => target.key === stored) ? [stored as PdfDocumentType] : ['ventas.estimate'];
    } catch {
  ...
  const [isResettingLayouts, setIsResettingLayouts] = useState(false);
  ...
  const [templateDefinition, setTemplateDefinition] = useState<PdfTemplateDefinition>(() => {
    let initialType: PdfDocumentType = 'ventas.estimate';
    try {
      const stored = window.localStorage.getItem(documentViewStorageKey);
      if (stored && PDF_TEMPLATE_TARGETS.some(target => target.key === stored)) initialType = stored as PdfDocumentType;
    } catch { /* almacenamiento no disponible */ }
    return createEditorDefaultDefinition(initialType, { ...DEFAULT_SETTINGS, companyName }, logo);
  });
  ...
  useEffect(() => {
    try { window.localStorage.setItem(documentViewStorageKey, documentType); } catch { /* almacenamiento no disponible */ }
  }, [documentType, documentViewStorageKey]);

  useEffect(() => {
    const target = getPdfTemplateTarget(documentType);
  ...
  const resetDraftForScope = useCallback((preferredDocumentType?: PdfDocumentType) => {
    let storedDocumentType: PdfDocumentType | undefined;
    try {
      const stored = window.localStorage.getItem(documentViewStorageKey);
      if (stored && PDF_TEMPLATE_TARGETS.some(target => target.key === stored)) storedDocumentType = stored as PdfDocumentType;
    } catch { /* almacenamiento no disponible */ }
    const target = getPdfTemplateTarget(preferredDocumentType || storedDocumentType || 'ventas.estimate');
    ...
    const normalizedSettings = normalizePdfPaperSettings(target.key, nextSettings) as typeof nextSettings;
    setActiveId('draft');
    ...
    setTemplateDefinition({ ...createEditorDefaultDefinition(target.key, normalizedSettings, logo), metadata: { preset: defaultTemplate.key } });
    setSelectedTemplateFieldId('company');
    setSourceMetadata(null);
  }, [companyName, corporateColor, documentViewStorageKey, logo]);
  const loadDesign = useCallback((record: PdfDocumentDesignRecord) => {
    ...
    const storedSettings = normalizePdfCompanySettings((record.settings || {}) as Record<string, unknown>) as Partial<PdfSettings>;
    ...
    const templateLogoUri = templateLogoTarget === assignedDocument
      ...
      : undefined;
    const mergedSettings = normalizePdfPaperSettings(assignedDocument, {
      ...DEFAULT_SETTINGS,
      companyName,
    ...
    }) as Partial<PdfSettings> & PdfSettings;
    ...
    const loadedDefinition = libraryPreset && !storedPreset
    ...
    const definition = ensureDashboardChartNodes(loadedDefinition, assignedDocument, mergedSettings);
  ...
  useEffect(() => {
    ...
    Promise.all([pdfDocumentDesignService.list(), pdfDocumentDesignService.listFolders()])
      .then(([records, folderRecords]) => {
        ...
        const preferredRecord = storedDocumentType && records?.find(record => normalizePdfTemplateKey(record.documentTypes?.[0] || '') === storedDocumentType);
        if (preferredRecord) loadDesign(preferredRecord);
        else if (storedDocumentType) resetDraftForScope(storedDocumentType);
        else if (records?.length) loadDesign(records[0]);
        else resetDraftForScope();
      })
      .catch(() => {})
      .finally(() => { if (!cancelled) setIsLoading(false); });
    return () => { cancelled = true; };
  }, [documentViewStorageKey, hasActiveBranchScope, loadDesign, resetDraftForScope, tenantId]);
  useEffect(() => {
  ...
  const selectedTargetEditable = canEditPdfTemplate(selectedTarget);
  const canModifyActive = hasActiveBranchScope && selectedTargetEditable && (activeId === 'draft' ? canCreate : canEdit);
  ...
  const availableDocumentTypes = DOCUMENTS.filter(item => !isPdfTemplateFixed(item) && !reservedDocumentDesign(item.key));
  const firstAvailableDocumentType = (preferred: PdfDocumentType, excludeId = activeId) => {
    const available = DOCUMENTS.filter(item => !isPdfTemplateFixed(item) && !reservedDocumentDesign(item.key, excludeId));
    return available.some(item => item.key === preferred) ? preferred : available[0]?.key || preferred;
  };

  const saveDesign = async () => {
    if (isPdfTemplateFixed(previewDocument)) { toast.error('Este formato físico se genera con un renderer fijo y no admite diseños editables.'); return; }
  ...
  const handleCanvasSettingsChange = (changes: Partial<PdfSettings>) => {
    if (!canModifyActive) return;
    const nextSettings = normalizePdfPaperSettings(previewDocument, { ...settings, ...changes }) as PdfSettings;
    setSettings(nextSettings);
  ...
  const moduleTargets = useMemo(() => DOCUMENTS.filter(target => target.module === selectedModule && (target.templateMode !== 'fixed' || target.key === previewDocument)), [previewDocument, selectedModule]);
  const selectTarget = (targetKey: string) => {
    const target = getPdfTemplateTarget(targetKey);
    if (isPdfTemplateFixed(target)) {
      toast.info('Este destino usa un formato físico fijo y no se edita desde Documentos PDF.');
      return;
    }
    const defaultTemplate = getDefaultLibraryTemplate();
    ...
    const nextSettings: PdfSettings = {
      ...
      ...defaultTemplate.settings,
      footerLayout: footerLayoutForTemplate(defaultTemplate.key),
      ...(!isSameTarget ? { paperSize: 'LETTER', orientation: 'portrait' } : {}),
      ...(target.family === 'cash-ticket' ? { paperSize: 'ROLL-80', orientation: 'portrait' } : {}),
      ...(target.family === 'cash-ticket' ? { primaryColor: '#000000', secondaryColor: '#000000', textColor: '#000000', lineColor: '#000000', paletteMode: 'independent' as const } : {}),
      ...(target.family === 'label' ? { paperSize: 'LABEL', orientation: 'landscape' } : {}),
      ...(isSameTarget ? {} : { logoUrl: undefined, templateLogoUrl: undefined, templateLogoUri: undefined, templateLogoTarget: undefined }),
    };
    const normalizedSettings = normalizePdfPaperSettings(target.key, nextSettings) as PdfSettings;
    setSelectedModule(target.module);
    setDocumentType(target.key);
    setAssignedDocuments([target.key]);
    setSettings(normalizedSettings);
    ...
    setSelectedTemplateKey(defaultTemplate.key);
    setName(`${defaultTemplate.name} · ${target.label}`);
    setDescription(`Diseño iniciado desde ${defaultTemplate.name} para ${target.label}; editable para esta sucursal.`);
  };
  const resetCurrentBranchLayouts = async () => {
    if (!hasActiveBranchScope || !canEdit || isResettingLayouts) return;
    setIsResettingLayouts(true);
    try {
      const result = await pdfDocumentDesignService.resetActiveLayouts();
      clearPdfDesignCache();
      const records = await pdfDocumentDesignService.list();
      setDesigns(records || []);
      const currentRecord = (records || []).find(record => record.id === activeId && record.isActive)
        || (records || []).find(record => record.isActive);
      if (currentRecord) loadDesign(currentRecord);
      else resetDraftForScope();
      toast.success(`Se restableció la composición de ${result.updatedCount} diseño(s) activo(s) de ${activeBranchLabel}.`);
    } catch (error: any) {
      toast.error(error?.message || 'No se pudieron restablecer los diseños activos de esta sucursal.');
    } finally {
      setIsResettingLayouts(false);
  ...
  return <div className="space-y-6">
    <Card data-tour="pdf-assignment" className="overflow-hidden border-primary/20 bg-gradient-to-br from-primary/10 via-card to-card">
      <CardContent className="space-y-4 p-4 md:p-5">
        <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
          ...
          <div className="flex flex-wrap items-center gap-2 text-[10px]"><Badge variant="outline" className={cn('gap-1.5', hasActiveBranchScope ? 'border-emerald-400/40 text-emerald-600 dark:text-emerald-300' : 'border-amber-400/50 text-amber-700 dark:text-amber-300')}><span className={cn('size-1.5 rounded-full', hasActiveBranchScope ? 'bg-emerald-500' : 'bg-amber-500')} />Sucursal: {hasActiveBranchScope ? activeBranchLabel : 'sin sucursal activa'}</Badge>{selectedFolder && <Badge variant="outline" className="gap-1.5"><Folder className="size-3" />{selectedFolder.name}</Badge>}{canEdit && <Button type="button" variant="outline" size="sm" className="h-8 gap-1.5" onClick={() => setIsResetConfirmOpen(true)} disabled={!hasActiveBranchScope || isResettingLayouts || !designs.some(record => record.isActive)}><RotateCcw className="size-3.5" />Restablecer activos</Button>}<Button variant="ghost" size="icon" className="size-8 shrink-0 rounded-lg text-muted-foreground" onClick={() => setShowTutorial(true)} aria-label="Cómo configurar documentos PDF" title="Cómo configurar documentos PDF"><WandSparkles className="size-4" /></Button></div>
        </div>
        <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-4">
          ...
          <Field label="Módulo"><select aria-label="Módulo del documento" value={selectedModule} onChange={event => { const moduleId = event.target.value as typeof selectedModule; setSelectedModule(moduleId); const first = DOCUMENTS.find(target => target.module === moduleId && !isPdfTemplateFixed(target) && !reservedDocumentDesign(target.key)); if (first) selectTarget(first.key); }} disabled={!canModifyActive} className="h-9 w-full cursor-pointer rounded-md border border-input bg-background px-3 text-sm"><option value="">Selecciona un módulo</option>{PDF_TEMPLATE_MODULES.map(module => <option key={module.id} value={module.id}>{module.label}</option>)}</select></Field>
    ...
    <div className="grid gap-5 xl:grid-cols-[270px_minmax(0,1fr)] [&_[data-tour=pdf-editor]]:hidden">
      ...
      <ConfirmDialog open={isResetConfirmOpen} onOpenChange={open => { if (!isResettingLayouts) setIsResetConfirmOpen(open); }} title="¿Restablecer diseños PDF activos?" description={`Se reemplazará la composición del canvas en todos los diseños activos de ${activeBranchLabel}. Se conservarán identidad, colores, carpetas y diseños inactivos. Esta acción no afecta otras sucursales.`} confirmLabel="Restablecer diseños" cancelLabel="Cancelar" loading={isResettingLayouts} onConfirm={resetCurrentBranchLayouts} />{showTutorial && <GuidedTour steps={PDF_DESIGN_TOUR_STEPS} onClose={() => setShowTutorial(false)} title="Documentos PDF" allowTargetInteraction />}
    </div>
  </div>;
}
