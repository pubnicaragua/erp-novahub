...
import { createDefaultTemplateDefinition, definitionFromExtractedPdf, sanitizeTemplateDefinition, type PdfTemplateDefinition } from '../../services/pdf-template-definition';
...
export function PdfDocumentCustomizer({ tenantId, companyName = '', corporateColor = '#10b981', logo, canEdit = true, canCreate = true, canDelete = true }: { tenantId?: string; companyName?: string; corporateColor?: string; logo?: string | null; canEdit?: boolean; canCreate?: boolean; canDelete?: boolean }) {
  ...
  const [templateDefinition, setTemplateDefinition] = useState<PdfTemplateDefinition>(() => createDefaultTemplateDefinition('ventas.estimate', { ...DEFAULT_SETTINGS, companyName }));
  ...
  const loadDesign = useCallback((record: PdfDocumentDesignRecord) => {
    ...
    const mergedSettings = { ...DEFAULT_SETTINGS, companyName, primaryColor: corporateColor, ...(TEMPLATE_LIBRARY.find(template => template.key === record.templateKey)?.settings || {}), ...(record.settings as Partial<PdfSettings>) };
    ...
    setTemplateDefinition(sanitizeTemplateDefinition((record.layoutZones as any)?.definition, assignedDocument, mergedSettings));
  ...
  }, [canEdit, companyName, corporateColor]);
  ...
  const saveDesign = async () => {
    ...
    setIsSaving(true);
    try {
      const payload = { name: name.trim(), description, documentTypes: assignedDocuments, folderId: selectedFolderId, settings, templateKey: selectedTemplateKey || 'custom', sourceType: sourceMetadata?.sourceType || 'SYSTEM', sourceFileUrl: sourceMetadata?.sourceFileUrl, sourceFileName: sourceMetadata?.sourceFileName, analysisStatus: sourceMetadata?.analysisStatus || 'NOT_APPLICABLE', layoutZones: { ...(sourceMetadata?.layoutZones || {}), definition: templateDefinition, fields: templateFields }, engine: 'HTML_TEMPLATE', isActive: true };
  ...
  const createDesign = () => { if (!canCreate) return; const nextDocumentType = firstAvailableDocumentType(documentType, 'draft'); const target = getPdfTemplateTarget(nextDocumentType); const nextSettings = { ...DEFAULT_SETTINGS, companyName, primaryColor: corporateColor }; setActiveId('draft'); setSelectedTemplateKey(''); setName('Nuevo diseño'); setDescription(''); setAssignedDocuments([nextDocumentType]); setDocumentType(nextDocumentType); setSelectedModule(target.module); setSourceMetadata(null); setTemplateFields(createTemplateFields(companyName)); setTemplateDefinition(createDefaultTemplateDefinition(nextDocumentType, nextSettings)); setSelectedTemplateFieldId('company'); setSettings(nextSettings); };
  ...
  const selectTarget = (targetKey: string) => { const target = getPdfTemplateTarget(targetKey); setSelectedModule(target.module); setDocumentType(target.key); setAssignedDocuments([target.key]); setTemplateDefinition(createDefaultTemplateDefinition(target.key, settings)); };
