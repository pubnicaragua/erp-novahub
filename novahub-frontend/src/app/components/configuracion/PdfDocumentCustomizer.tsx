...
type PdfSettings = {
  paperSize: 'LETTER' | 'A4' | 'OFICIO' | 'LEGAL'; orientation: 'portrait' | 'landscape';
...
export function PdfDocumentCustomizer({ tenantId, companyName = '', corporateColor = '#10b981', logo, canEdit = true, canCreate = true, canDelete = true }: { tenantId?: string; companyName?: string; corporateColor?: string; logo?: string | null; canEdit?: boolean; canCreate?: boolean; canDelete?: boolean }) {
  ...
  const applyTemplate = (template: typeof TEMPLATE_LIBRARY[number]) => { if (!canModifyActive) return; setSelectedTemplateKey(template.key); setSettings(prev => ({ ...prev, ...template.settings })); toast.success(`Plantilla ${template.name} aplicada`); };
