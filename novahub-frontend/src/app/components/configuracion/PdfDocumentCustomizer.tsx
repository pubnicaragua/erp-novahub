...
export function PdfDocumentCustomizer({ tenantId, companyName = '', corporateColor = '#10b981', logo }: { tenantId?: string; companyName?: string; corporateColor?: string; logo?: string | null }) {
  ...
  const createDesign = () => { const nextDocumentType = firstAvailableDocumentType(documentType, 'draft'); const target = getPdfTemplateTarget(nextDocumentType); setActiveId('draft'); setSelectedTemplateKey(''); setName('Nuevo diseño'); setDescription(''); setAssignedDocuments([nextDocumentType]); setDocumentType(nextDocumentType); setSelectedModule(target.module); setSourceMetadata(null); setTemplateFields(createTemplateFields(companyName)); setSelectedTemplateFieldId('company'); setSettings({ ...DEFAULT_SETTINGS, companyName, primaryColor: corporateColor }); };
  ...
  const uploadCustomTemplate = async () => {
    if (!pendingTemplateFile || !templateName.trim()) { toast.error('Ingresa un nombre para la plantilla'); return; }
