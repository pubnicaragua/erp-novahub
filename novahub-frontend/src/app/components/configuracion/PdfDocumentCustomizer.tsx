...
type PdfSettings = {
  paperSize: 'LETTER' | 'A4' | 'LEGAL'; orientation: 'portrait' | 'landscape';
  headerLayout: 'classic' | 'split' | 'banner' | 'compact' | 'ribbon' | 'topline' | 'sidebar' | 'centered' | 'boxed' | 'corner' | 'editorial' | 'double-band';
  ...
  fontSize: number; primaryColor: string; textColor: string; lineColor: string; separator: 'solid' | 'dashed' | 'none';
  paletteMode: 'corporate' | 'independent';
};

const DEFAULT_SETTINGS: PdfSettings = {
  paperSize: 'LETTER', orientation: 'portrait', headerLayout: 'split', tableLayout: 'standard', logoPosition: 'left', logoSize: 34,
  showCompanyName: true, companyName: '', slogan: 'Soluciones simples para crecer', fiscalInfo: 'RUC / Identificación fiscal',
  address: 'Dirección fiscal de la empresa', phone: '+505 0000-0000', email: 'contacto@empresa.com', website: 'www.empresa.com',
  bankInfo: '', showQr: false, showBarcode: false, watermark: '', watermarkOpacity: 12, footerText: 'Gracias por confiar en nosotros.',
  showPageNumber: true, pageNumberFormat: 'page-of', pageNumberCustom: 'Página {page} de {pages}', legalText: '', terms: '', defaultNotes: '',
  margins: 14, fontFamily: 'helvetica', fontSize: 9, primaryColor: '#10b981', textColor: '#334155', lineColor: '#e2e8f0', separator: 'solid', paletteMode: 'corporate',
};

const TEMPLATE_LIBRARY = [
  { key: 'classic', name: 'Clásico', description: 'Jerarquía tradicional y sobria', color: '#0f766e', settings: { headerLayout: 'classic', tableLayout: 'standard', separator: 'solid', primaryColor: '#0f766e', textColor: '#334155', lineColor: '#cbd5e1', paletteMode: 'independent' } },
  { key: 'modern', name: 'Moderno', description: 'Banda sólida y alto contraste', color: '#2563eb', settings: { headerLayout: 'banner', tableLayout: 'striped', separator: 'none', primaryColor: '#2563eb', textColor: '#172554', lineColor: '#bfdbfe', paletteMode: 'independent' } },
  { key: 'minimal', name: 'Minimalista', description: 'Mucho aire, máxima legibilidad', color: '#64748b', settings: { headerLayout: 'compact', tableLayout: 'minimal', separator: 'none', fontSize: 8, primaryColor: '#64748b', textColor: '#475569', lineColor: '#e2e8f0', paletteMode: 'independent' } },
  { key: 'corporate', name: 'Corporativo', description: 'Ordenado para procesos empresariales', color: '#7c3aed', settings: { headerLayout: 'split', tableLayout: 'boxed', separator: 'solid', primaryColor: '#7c3aed', textColor: '#312e81', lineColor: '#ddd6fe', paletteMode: 'independent' } },
  { key: 'elegant', name: 'Elegante', description: 'Detalle editorial y refinado', color: '#be7b35', settings: { headerLayout: 'editorial', tableLayout: 'minimal', fontFamily: 'times', separator: 'dashed', primaryColor: '#be7b35', textColor: '#4b3621', lineColor: '#ead7bd', paletteMode: 'independent' } },
  { key: 'industrial', name: 'Industrial', description: 'Fuerte, directo y operativo', color: '#475569', settings: { headerLayout: 'ribbon', tableLayout: 'ledger', fontFamily: 'courier', separator: 'solid', primaryColor: '#475569', textColor: '#1e293b', lineColor: '#94a3b8', paletteMode: 'independent' } },
  { key: 'ocean', name: 'Oceánico', description: 'Franja azul con datos equilibrados', color: '#0369a1', settings: { headerLayout: 'double-band', tableLayout: 'striped', primaryColor: '#0369a1', textColor: '#0c4a6e', lineColor: '#bae6fd', paletteMode: 'independent' } },
  { key: 'forest', name: 'Bosque', description: 'Verde profundo y tabla natural', color: '#166534', settings: { headerLayout: 'topline', tableLayout: 'accent', primaryColor: '#166534', textColor: '#14532d', lineColor: '#bbf7d0', paletteMode: 'independent' } },
  { key: 'sunset', name: 'Atardecer', description: 'Naranja cálido y bloques suaves', color: '#ea580c', settings: { headerLayout: 'corner', tableLayout: 'cards', primaryColor: '#ea580c', textColor: '#431407', lineColor: '#fed7aa', paletteMode: 'independent' } },
  { key: 'lavender', name: 'Lavanda', description: 'Suave, creativo y contemporáneo', color: '#9333ea', settings: { headerLayout: 'centered', tableLayout: 'minimal', primaryColor: '#9333ea', textColor: '#581c87', lineColor: '#e9d5ff', paletteMode: 'independent' } },
  { key: 'graphite', name: 'Grafito', description: 'Monocromo con acentos precisos', color: '#374151', settings: { headerLayout: 'boxed', tableLayout: 'boxed', primaryColor: '#374151', textColor: '#111827', lineColor: '#d1d5db', paletteMode: 'independent' } },
  { key: 'aqua', name: 'Aqua', description: 'Ligero, fresco y tecnológico', color: '#0891b2', settings: { headerLayout: 'sidebar', tableLayout: 'striped', primaryColor: '#0891b2', textColor: '#164e63', lineColor: '#a5f3fc', paletteMode: 'independent' } },
  { key: 'wine', name: 'Vino', description: 'Serio, elegante y con carácter', color: '#9f1239', settings: { headerLayout: 'banner', tableLayout: 'compact', primaryColor: '#9f1239', textColor: '#4c0519', lineColor: '#fecdd3', paletteMode: 'independent' } },
  { key: 'slate', name: 'Pizarra', description: 'Equilibrio visual para operaciones', color: '#334155', settings: { headerLayout: 'topline', tableLayout: 'ledger', primaryColor: '#334155', textColor: '#1e293b', lineColor: '#cbd5e1', paletteMode: 'independent' } },
  { key: 'paper', name: 'Papel', description: 'Editorial, claro y muy limpio', color: '#78716c', settings: { headerLayout: 'editorial', tableLayout: 'minimal', fontFamily: 'Georgia', primaryColor: '#78716c', textColor: '#44403c', lineColor: '#d6d3d1', paletteMode: 'independent' } },
  { key: 'neon', name: 'Neón', description: 'Acentos vivos para documentos digitales', color: '#16a34a', settings: { headerLayout: 'sidebar', tableLayout: 'accent', primaryColor: '#16a34a', textColor: '#14532d', lineColor: '#bbf7d0', paletteMode: 'independent' } },
  { key: 'horizon', name: 'Horizonte', description: 'Cabecera amplia y datos alineados', color: '#1d4ed8', settings: { headerLayout: 'double-band', tableLayout: 'boxed', primaryColor: '#1d4ed8', textColor: '#1e3a8a', lineColor: '#bfdbfe', paletteMode: 'independent' } },
  { key: 'fluid', name: 'Fluido', description: 'Ritmo visual con curvas y espacios', color: '#0d9488', settings: { headerLayout: 'ribbon', tableLayout: 'cards', primaryColor: '#0d9488', textColor: '#134e4a', lineColor: '#99f6e4', paletteMode: 'independent' } },
  { key: 'frames', name: 'Recuadros', description: 'Módulos delimitados para cada sección', color: '#b45309', settings: { headerLayout: 'boxed', tableLayout: 'cards', primaryColor: '#b45309', textColor: '#451a03', lineColor: '#fde68a', paletteMode: 'independent' } },
  { key: 'wave', name: 'Ola', description: 'Banda dinámica y tabla destacada', color: '#4f46e5', settings: { headerLayout: 'corner', tableLayout: 'accent', primaryColor: '#4f46e5', textColor: '#312e81', lineColor: '#c7d2fe', paletteMode: 'independent' } },
] as const;
...
const PDF_DESIGN_TOUR_STEPS: GuidedTourStep[] = [
  ...
  { target: '[data-tour="pdf-library"]', title: 'Biblioteca de plantillas', description: 'Explora 20 bases visuales con encabezados, tablas y paletas distintas. Al seleccionar una, todos sus estilos pasan al diseño actual y luego puedes afinarlos.', placement: 'right' },
  ...
  { target: '[data-tour="pdf-preview"]', title: 'Vista previa en vivo', description: 'Esta hoja representa el documento final. Cada cambio que hagas en el editor lateral se refleja inmediatamente aquí, sin guardar ni generar el PDF.', placement: 'left' },
  ...
  { target: '[data-tour="pdf-editor"]', title: 'Editor del diseño', description: 'Desde este panel configuras la identidad, encabezado, contenido, pie de página, textos legales, papel, márgenes, tipografía y colores.', placement: 'left' },
...
function PaperPreview({ settings, documentType, companyName, logo }: { settings: PdfSettings; documentType: PdfDocumentType; companyName: string; logo?: string | null }) {
  ...
  return <div className={cn('mx-auto w-full max-w-[520px] overflow-hidden rounded-md border bg-white text-slate-700 shadow-2xl', pageClass)} style={{ fontFamily: settings.fontFamily, fontSize: `${settings.fontSize}px` }}>
    ...
    <div className="h-full p-[5%]" style={{ paddingLeft: `${Math.max(3, settings.margins / 2)}%`, paddingRight: `${Math.max(3, settings.margins / 2)}%` }}>
      <div className={cn('flex min-w-0 flex-nowrap min-h-[16%] justify-between gap-4 border-b pb-3', bannerHeader && 'rounded-md border-0 p-3 text-white', settings.headerLayout === 'ribbon' && 'rounded-br-[3rem]', settings.headerLayout === 'corner' && 'rounded-bl-[3rem]', settings.headerLayout === 'compact' && 'min-h-0', settings.headerLayout === 'topline' && 'border-t-4 pt-2', settings.headerLayout === 'sidebar' && 'border-l-4 pl-3', centeredHeader && 'text-center', boxedHeader && 'rounded-lg border p-3', align)} style={{ borderColor: settings.lineColor, background: bannerHeader ? primary : undefined }}>
        <div className={cn('flex min-w-0 flex-col gap-1', align)}>
          {logo ? <img src={logo} alt="Logo" className="mb-1 object-contain" style={{ width: `${Math.min(settings.logoSize, 60)}px`, height: `${Math.min(settings.logoSize / 2, 32)}px` }} /> : <div className="flex size-8 items-center justify-center rounded bg-slate-100"><ImagePlus className="size-4 text-slate-400" /></div>}
...
function StructuredDocumentPreview({ target, settings, companyName, logo }: { target: PdfTemplateTarget; settings: PdfSettings; companyName: string; logo?: string | null }) {
  ...
  return <div className={cn('mx-auto w-full max-w-[520px] overflow-hidden rounded-md border bg-white text-slate-700 shadow-2xl', pageClass)} style={{ fontFamily: settings.fontFamily, fontSize: `${settings.fontSize}px` }}>
    <div className="h-full p-[7%]" style={{ paddingLeft: `${Math.max(3, settings.margins / 2)}%`, paddingRight: `${Math.max(3, settings.margins / 2)}%` }}>
      <div className="flex min-w-0 flex-nowrap items-start justify-between gap-4 border-b pb-4" style={{ borderColor: settings.lineColor }}>
        <div className="min-w-0">{logo ? <img src={logo} alt="Logo" className="mb-2 object-contain" style={{ width: `${Math.min(settings.logoSize, 60)}px`, height: `${Math.min(settings.logoSize / 2, 32)}px` }} /> : <div className="mb-2 flex size-8 items-center justify-center rounded bg-slate-100"><ImagePlus className="size-4 text-slate-400" /></div>}{settings.showCompanyName && <strong className="block text-sm" style={{ color: primary }}>{settings.companyName || companyName || 'Mi Empresa'}</strong>}<span className="text-[.75em] opacity-60">{settings.fiscalInfo}</span></div>
...
export function PdfDocumentCustomizer({ tenantId, companyName = '', corporateColor = '#10b981', logo, canEdit = true, canCreate = true, canDelete = true }: { tenantId?: string; companyName?: string; corporateColor?: string; logo?: string | null; canEdit?: boolean; canCreate?: boolean; canDelete?: boolean }) {
  ...
  const [name, setName] = useState('Diseño principal');
  const [description, setDescription] = useState('Diseño corporativo para documentos comerciales');
  ...
  const canModifyActive = activeId === 'draft' ? canCreate : canEdit;
  ...
  const saveDesign = async () => {
    ...
    setIsSaving(true);
    try {
      const payload = { name: name.trim(), description, documentTypes: assignedDocuments, folderId: selectedFolderId, settings, templateKey: selectedTemplateKey || 'custom', sourceType: sourceMetadata?.sourceType || 'SYSTEM', sourceFileUrl: sourceMetadata?.sourceFileUrl, sourceFileName: sourceMetadata?.sourceFileName, analysisStatus: sourceMetadata?.analysisStatus || 'NOT_APPLICABLE', layoutZones: { ...(sourceMetadata?.layoutZones || {}), fields: templateFields }, engine: isUploadedSource ? 'HTML_TEMPLATE' : 'NATIVE_PDF', isActive: true };
  ...
  const createDesign = () => { if (!canCreate) return; const nextDocumentType = firstAvailableDocumentType(documentType, 'draft'); const target = getPdfTemplateTarget(nextDocumentType); setActiveId('draft'); setSelectedTemplateKey(''); setName('Nuevo diseño'); setDescription(''); setAssignedDocuments([nextDocumentType]); setDocumentType(nextDocumentType); setSelectedModule(target.module); setSourceMetadata(null); setTemplateFields(createTemplateFields(companyName)); setSelectedTemplateFieldId('company'); setSettings({ ...DEFAULT_SETTINGS, companyName, primaryColor: corporateColor }); };
  ...
  const applyTemplate = (template: typeof TEMPLATE_LIBRARY[number]) => { if (!canModifyActive) return; const structure: Partial<PdfSettings> = { headerLayout: template.settings.headerLayout, tableLayout: template.settings.tableLayout, separator: template.settings.separator, ...(template.settings.fontFamily ? { fontFamily: template.settings.fontFamily } : {}), ...(template.settings.fontSize ? { fontSize: template.settings.fontSize } : {}) }; setSelectedTemplateKey(template.key); setSettings(prev => ({ ...prev, ...structure })); toast.success(`Estructura ${template.name} aplicada`); };
  ...
  const uploadCustomTemplate = async () => {
    if (!canCreate || !pendingTemplateFile || !templateName.trim()) { if (canCreate) toast.error('Ingresa un nombre para la plantilla'); return; }
    ...
    try {
      ...
      const uploaded = await storageService.uploadFile('documents', pendingTemplateFile, { folder: 'pdf-templates', scopeId: tenantId });
      const extractedLayout = isHtml ? null : await extractPdfLayout(pendingTemplateFile);
      const payload = isHtml
        ? { name: templateName.trim(), description: `HTML pdf2htmlEX conservado desde: ${pendingTemplateFile.name}`, documentTypes: [uploadDocumentType], folderId: selectedFolderId, settings, templateKey: 'custom', sourceType: 'UPLOADED_HTML' as const, sourceFileUrl: uploaded.uri, sourceFileName: pendingTemplateFile.name, analysisStatus: 'HTML_CONVERTED', layoutZones: { status: 'html-uploaded', htmlTemplate: { version: 2, kind: 'pdf2htmlEX', uri: uploaded.uri, provider: 'uploaded-html' }, fields: templateFields }, engine: 'HTML_TEMPLATE', isActive: true }
        : { name: templateName.trim(), description: `Diseño conservado desde: ${pendingTemplateFile.name}`, documentTypes: [uploadDocumentType], folderId: selectedFolderId, settings, templateKey: 'custom', sourceType: 'UPLOADED_PDF' as const, sourceFileUrl: uploaded.uri, sourceFileName: pendingTemplateFile.name, analysisStatus: 'LAYOUT_EXTRACTED', layoutZones: { status: 'extracted', expectedZones: ['header', 'body', 'table', 'footer', 'watermark'], extractedLayout, fields: templateFields }, engine: 'HTML_TEMPLATE', isActive: true };
      const created = await pdfDocumentDesignService.create(payload);
      const converted = isHtml ? created : await pdfDocumentDesignService.convertToHtml(created.id).catch(() => created);
      setDesigns(prev => [converted, ...prev]); setActiveId(converted.id); setAssignedDocuments([uploadDocumentType]); setDocumentType(uploadDocumentType); setPendingTemplateDocumentType(null); setName(converted.name); setDescription(converted.description || ''); setSourceMetadata({ sourceType: converted.sourceType, sourceFileUrl: converted.sourceFileUrl || undefined, sourceFileName: converted.sourceFileName || undefined, analysisStatus: converted.analysisStatus, layoutZones: converted.layoutZones || undefined }); setTemplateModalOpen(false);
      toast.success(isHtml ? 'HTML pdf2htmlEX cargado y agregado a Mis diseños' : converted.analysisStatus === 'HTML_CONVERTED' ? 'Plantilla convertida a HTML y agregada a Mis diseños' : 'Plantilla agregada. Se conservará usando el visor PDF original.');
    } catch (error: any) { toast.error(error?.message || 'No se pudo analizar o cargar la plantilla'); } finally { setIsUploadingTemplate(false); }
  };
  const openNewFolder = () => { if (!canCreate) return; setEditingFolderId(null); setFolderName(''); setFolderDialogOpen(true); };
  ...
  const uploadLogo = async (file: File) => { if (!canModifyActive) return; if (!file.type.startsWith('image/')) { toast.error('Selecciona una imagen para el logotipo'); return; } try { const uploaded = await storageService.uploadFile('tenant-branding', file, { folder: 'pdf-logos', scopeId: tenantId }); update('logoUrl', uploaded.url); toast.success('Logotipo agregado al diseño'); } catch (error: any) { toast.error(error?.message || 'No se pudo cargar el logotipo'); } };
  ...
  return <div className="space-y-6">
    <Card className="overflow-hidden border-primary/20 bg-gradient-to-br from-primary/10 via-card to-card"><CardContent className="flex flex-col gap-4 p-5 md:flex-row md:items-center md:justify-between"><div className="flex items-start gap-3"><div className="flex size-11 shrink-0 items-center justify-center rounded-2xl bg-primary/15 text-primary"><FileCog className="size-6" /></div><div><h2 className="text-xl font-black">Personalización de Documentos PDF</h2><p className="mt-1 max-w-3xl text-sm text-muted-foreground">Diseña una vez y aplica el mismo lenguaje visual a cotizaciones, facturas y cualquier documento exportable del ERP.</p></div></div><Button variant="outline" className="shrink-0 cursor-pointer gap-2" onClick={() => setShowTutorial(true)}><WandSparkles className="size-4" />Tutorial</Button></CardContent></Card>
    <div className="grid gap-5 xl:grid-cols-[250px_minmax(360px,1fr)_360px]">
      <div className="space-y-5">
        ...
        <Card data-tour="pdf-library"><button type="button" onClick={() => setLibraryOpen(open => !open)} className="flex w-full cursor-pointer items-center justify-between px-4 py-4 text-left"><span className="flex items-center gap-2 text-base font-black"><LayoutTemplate className="size-4 text-primary" />Biblioteca <Badge variant="outline" className="text-[10px]">20 opciones</Badge></span><ChevronDown className={cn('size-4 text-muted-foreground transition-transform', libraryOpen && 'rotate-180')} /></button>{libraryOpen && <><CardHeader className="border-t border-border/40 pb-3 pt-3"><CardDescription>Selecciona una estructura de encabezado y tabla. Los colores se configuran aparte en el editor.</CardDescription></CardHeader><CardContent className="space-y-3 pt-0"><select aria-label="Seleccionar estructura de plantilla" value={selectedTemplateKey} onChange={event => { const template = TEMPLATE_LIBRARY.find(item => item.key === event.target.value); if (template) applyTemplate(template); }} className="h-10 w-full cursor-pointer rounded-lg border border-primary/40 bg-background px-3 text-sm font-semibold"><option value="">Selecciona una plantilla</option>{TEMPLATE_LIBRARY.map(template => <option key={template.key} value={template.key}>{template.name}</option>)}</select>{selectedTemplateKey && (() => { const selected = TEMPLATE_LIBRARY.find(template => template.key === selectedTemplateKey); return selected ? <div className="rounded-xl border border-primary/40 bg-primary/5 p-3"><p className="text-sm font-bold text-primary">{selected.name}</p><div className="mt-2 flex flex-wrap gap-1.5"><Badge variant="outline" className="text-[9px]">Header: {selected.settings.headerLayout}</Badge><Badge variant="outline" className="text-[9px]">Tabla: {selected.settings.tableLayout}</Badge></div></div> : null; })()}</CardContent></>}</Card>
        <Card data-tour="pdf-upload" className="border-dashed"><CardContent className="space-y-3 p-4"><div className="flex items-center justify-between gap-2"><div className="flex items-center gap-2"><Upload className="size-4 text-muted-foreground" /><p className="text-sm font-bold">Importar plantilla</p></div><Badge variant="outline" className="text-amber-500">En desarrollo</Badge></div><p className="text-[11px] text-muted-foreground">La importación de PDF/HTML está temporalmente desactivada mientras terminamos el motor de conversión editable.</p><Button variant="outline" disabled className="w-full cursor-not-allowed gap-2"><Upload className="size-4" />Disponible próximamente</Button></CardContent></Card>
      </div>

      <Card data-tour="pdf-preview" className="min-w-0 overflow-hidden bg-muted/10"><CardHeader className="gap-3 border-b border-border/40 pb-4"><div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between"><div><CardTitle className="flex items-center gap-2 text-base"><FileText className="size-4 text-primary" />Vista previa en vivo</CardTitle><CardDescription>{isUploadedSource ? 'Vista fiel del diseño cargado. Selecciona una zona para vincularla y ajustar su posición.' : 'Los cambios se reflejan al instante en el documento.'}</CardDescription></div><Badge variant="outline" className="w-fit gap-1.5"><span className="size-1.5 rounded-full bg-primary" />{sourceMetadata?.analysisStatus === 'HTML_CONVERTED' ? 'HTML real' : isUploadedSource ? 'Diseño cargado' : 'Sin guardar'}</Badge></div><div data-tour="pdf-document-selector" className="flex items-center justify-between rounded-lg border border-primary/20 bg-primary/5 px-3 py-2"><span className="text-[10px] font-black uppercase tracking-wider text-muted-foreground">Vista asignada</span><span className="text-xs font-bold text-primary">{getPdfTemplateTarget(previewDocument).moduleLabel} · {documentLabel(previewDocument)}</span></div></CardHeader><CardContent className="flex min-h-[760px] items-start justify-center overflow-auto p-5 md:p-8">{isUploadedSource ? <UploadedTemplatePreview source={sourceMetadata.sourceFileUrl || null} htmlUri={(sourceMetadata.layoutZones as any)?.htmlTemplate?.kind === 'pdf2htmlEX' ? (sourceMetadata.layoutZones as any)?.htmlTemplate?.uri : undefined} htmlPageSize={(sourceMetadata.layoutZones as any)?.htmlTemplate?.pageSizes?.[0]} fields={liveTemplateFields} selectedFieldId={selectedTemplateFieldId} onSelectField={setSelectedTemplateFieldId} onUpdateField={updateTemplateField} fieldValue={templateFieldValue} /> : <StructuredDocumentPreview target={getPdfTemplateTarget(previewDocument)} settings={settings} companyName={companyName} logo={settings.logoUrl || logo} />}</CardContent></Card>

      <Card data-tour="pdf-editor" className="flex min-h-0 max-h-[850px] min-w-0 flex-col"><CardHeader className="shrink-0 border-b border-border/40 pb-4"><div className="flex items-start justify-between gap-3"><div><CardTitle className="text-base">Editor del diseño</CardTitle><CardDescription>Configura el documento y sus asignaciones.</CardDescription></div><Button size="icon" variant="outline" className="size-8 cursor-pointer" onClick={saveDesign} disabled={isSaving}>{isSaving ? <Loader2 className="size-4 animate-spin" /> : <Save className="size-4" />}</Button></div></CardHeader><CardContent className="min-h-0 flex-1 overflow-y-auto pr-3">
