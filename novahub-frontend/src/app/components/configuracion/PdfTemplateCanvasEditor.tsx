...
function nodeStyle(node: PdfTemplateNode, settings: CanvasSettings) {
  ...
  return {
    ...
    borderColor: node.borderColor || settings.lineColor || '#e2e8f0', borderStyle, borderWidth: borderStyle === 'none' ? '0px' : borderStyle === 'double' ? '3px' : node.type === 'image' ? '2px' : '1px', borderRadius,
    clipPath, opacity: node.opacity ?? 1,
    overflow: node.type === 'image' ? 'hidden' : undefined,
