...
import { createPdfTemplateSampleData, getTemplateTokenSample, PDF_DEFAULT_FONT_SCALE, pdfTemplateImageShapeStyles, resolveTemplateToken, TEMPLATE_TOKENS, type PdfTemplateChart, type PdfTemplateColumn, type PdfTemplateData, type PdfTemplateDefinition, type PdfTemplateNode, type PdfTemplateNodeType, type PdfTemplateReportSection, type PdfTemplateReportSectionStyle } from '../../services/pdf-template-definition';
...
function LogoPreview({ src, companyName, className, primaryColor, secondaryColor, style, shape, borderStyle, borderColor, borderWidth }: { src?: string | null; companyName?: string; className?: string; primaryColor?: string; secondaryColor?: string; style?: CSSProperties; shape?: PdfTemplateNode['shape']; borderStyle?: PdfTemplateNode['borderStyle']; borderColor?: string; borderWidth?: string }) {
  ...
  const isAngular = shape === 'angled';
  const dashArray = borderStyle === 'dashed' ? '5 3' : borderStyle === 'dotted' ? '0.1 3' : undefined;
  return <div className={cn('relative box-border overflow-hidden', className)} style={style}>
    {!src || failed
      ? <div className="absolute inset-0 flex items-center justify-center rounded-[22%] text-[11px] font-black tracking-wide text-white" style={{ background: `linear-gradient(135deg, ${primaryColor || '#10b981'}, ${secondaryColor || '#0f3b65'})` }} aria-label="Identificador de empresa">{initials}</div>
      : <img src={src} alt="Logotipo de la empresa" className="absolute inset-0 h-full w-full object-contain" onError={() => setFailed(true)} />}
    {isAngular && borderStyle && borderStyle !== 'none' && <svg aria-hidden="true" viewBox="0 0 100 100" preserveAspectRatio="none" className="pointer-events-none absolute inset-0 h-full w-full">
      {borderStyle === 'double' ? <>
        <polygon points="1.5,1.5 98.5,1.5 86.8,98.5 1.5,98.5" fill="none" stroke={borderColor || '#e2e8f0'} strokeWidth="1.2" />
        <polygon points="5,5 94,5 83,95 5,95" fill="none" stroke={borderColor || '#e2e8f0'} strokeWidth="1.2" />
      </> : <polygon points="1.5,1.5 98.5,1.5 86.8,98.5 1.5,98.5" fill="none" stroke={borderColor || '#e2e8f0'} strokeWidth={borderWidth === '0px' ? 0 : 2} strokeDasharray={dashArray} strokeLinecap={borderStyle === 'dotted' ? 'round' : undefined} />}
    </svg>}
  </div>;
}
...
function nodeShapeStyles(node: PdfTemplateNode) {
  ...
  return { borderRadius, clipPath };
}

function logoPreviewStyle(node: PdfTemplateNode, settings: CanvasSettings): CSSProperties {
  const { borderRadius, clipPath } = nodeShapeStyles(node);
  const borderStyle = node.borderStyle || 'none';
  const borderWidth = borderStyle === 'none' ? '0px' : borderStyle === 'double' ? '3px' : '2px';
  return {
    width: '100%', height: '100%', boxSizing: 'border-box', overflow: 'hidden',
    borderRadius, clipPath, backgroundColor: node.backgroundColor || 'transparent',
    border: node.shape === 'angled' ? '0' : `${borderWidth} ${borderStyle} ${node.borderColor || settings.lineColor || '#e2e8f0'}`,
  };
}
...
function nodeStyle(node: PdfTemplateNode, settings: CanvasSettings) {
  ...
  return {
    ...
    color: node.color || settings.textColor || '#334155', backgroundColor: isImage ? 'transparent' : node.backgroundColor || 'transparent',
    borderColor: node.borderColor || settings.lineColor || '#e2e8f0', borderStyle: isImage ? 'none' : borderStyle, borderWidth: isImage ? '0px' : borderStyle === 'none' ? '0px' : borderStyle === 'double' ? '3px' : '1px', borderRadius: isImage ? '0px' : borderRadius,
    clipPath: isImage ? 'none' : clipPath, opacity: node.opacity ?? 1,
    overflow: undefined,
...
function TablePreview({ node, settings, data, reportSection }: { node: PdfTemplateNode; settings: CanvasSettings; data: PdfTemplateData; reportSection?: PdfTemplateReportSection }) {
  ...
  const rowTextColor = node.tableTextColor || node.color || settings.textColor || '#334155';
  ...
  return <div className="h-full overflow-hidden rounded-[inherit] border" style={{ borderColor: node.borderColor }}>
    ...
    {rows.slice(0, 4).map((row, index) => <div key={index} className="flex border-b px-2 py-1 last:border-0" style={{ color: rowTextColor, fontSize: canvasFontSize(Math.max(7, (Number(node.fontSize || settings.fontSize || 9) || 9) - 1)), backgroundColor: node.tableRowColor || (index % 2 && ['standard', 'striped', 'accent'].includes(settings.tableLayout || '') ? stripeColor : 'transparent') }}>
...
function ReportSectionsPreview({ node, settings, data, selectedSectionIndex, onSelectSection }: { node: PdfTemplateNode; settings: CanvasSettings; data: PdfTemplateData; selectedSectionIndex?: number | null; onSelectSection?: (sectionIndex: number) => void }) {
  ...
  return <div className="h-full overflow-hidden rounded-[inherit] border" style={{ borderColor: node.borderColor }}>
    ...
    {previewSections.map(({ section, sectionIndex }, visibleIndex) => {
      ...
      const sectionRowTextColor = sectionStyle.rowTextColor || node.tableTextColor || node.color || settings.textColor || '#334155';
      ...
      <div className="overflow-hidden rounded-[2px] border" style={{ borderColor: node.borderColor }}>
        ...
        {section.rows.slice(0, 3).map((row, rowIndex) => <div key={rowIndex} className="flex border-b px-1.5 py-1 last:border-0" style={{ color: sectionRowTextColor, fontSize: canvasFontSize(6.7), backgroundColor: sectionRowColor || (rowIndex % 2 && !compactTable ? sectionStripeColor : 'transparent') }}>
  ...
  return <>
    ...
    {selectedSection && selectedSectionIndex !== null && <div className="rounded-xl border border-emerald-400/30 bg-emerald-400/5 p-3">
      ...
      <div className="mt-3 grid grid-cols-2 gap-2">
        ...
        <div><Label className="text-[10px] text-slate-400">Texto de las filas</Label><FastColorInput value={selectedStyle?.rowTextColor || node.tableTextColor || node.color || '#334155'} onChange={value => onPatchStyle({ rowTextColor: value })} className="mt-1 h-8 w-full cursor-pointer border-slate-700 bg-slate-900 p-1" /></div>
