...
export const PushView: React.FC<PushViewProps> = ({ data, loading, onRefresh }) => {
  ...
  const columns: ColumnDef<PushNotification>[] = [
    ...
    { key: 'body', header: 'Contenido', width: '40%', editable: true },
    ...
    { key: 'sentAt', header: 'Fecha', width: '150px', type: 'date', render: (val: any) => val ? format(new Date(val), 'MMM dd, HH:mm') : '-' },
  ];
  ...
  const handleAdd = async () => {
    try {
      await pushNotificationsService.create({ title: 'Nueva Notificación Push', body: 'Contenido...', type: 'SYSTEM', sent: false });
  ...
  const kpis = [
    ...
    { title: 'Dispositivos',    value: new Set(data.map(p => p.deviceId).filter(Boolean)).size || '-',  icon: Smartphone,    color: 'text-purple-500',  bg: 'bg-purple-500/10'  },
  ...
  const filtered = data.filter(p => p.title?.toLowerCase().includes(searchTerm.toLowerCase()) || p.body?.toLowerCase().includes(searchTerm.toLowerCase()));
