...
const HELP_DATA: Record<string, {
  ...
  'diario': {
    ...
    faq: [
      ...
      { q: '¿Cómo se vincula con facturas?', a: 'Las facturas pendientes no generan asiento. Cuando una factura queda pagada, se crea un único asiento: débito a la cuenta real del cobro, crédito a IVA y crédito a Ingresos por Ventas netos del costo de inventario, junto con la salida del inventario.' },
    ],
  },
  'libro-mayor': {
    ...
    <div className="accounting-module flex h-full min-h-[calc(100vh-5rem)] min-w-0 overflow-x-hidden">
      <main className="min-w-0 flex-1 overflow-y-auto custom-scrollbar">
        <div className="mx-auto min-w-0 w-full max-w-[1700px] overflow-x-hidden p-3 pb-20 sm:p-6 md:px-10 md:pb-20 md:pt-4">
          ...
          <div className={cn(
            'accounting-tabs-scroll mb-4 w-full min-w-0 overflow-x-auto custom-scrollbar',
            !isSidebarCollapsed && 'hidden lg:hidden',
          )}>
            <div className="erp-module-tabs-rail flex w-max min-w-full gap-1.5 rounded-lg border border-border bg-muted p-1">
              {visibleSections.map((section) => {
                ...
                return (
                  <button
                    ...
                    className={cn(
                      'flex min-h-9 min-w-10 shrink-0 items-center justify-center gap-2 rounded-md border border-transparent px-2 py-2.5 text-xs font-semibold uppercase tracking-widest whitespace-nowrap transition-[background-color,color,box-shadow,transform] duration-200 sm:min-w-0 sm:justify-start sm:px-3',
                      isActive
                        ? 'bg-primary/[0.12] text-foreground shadow-[inset_0_-2px_0_var(--primary)]'
