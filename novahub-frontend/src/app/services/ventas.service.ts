...
export const customersService = {
  ...
  getLookup: (filters?: ApiFilters, signal?: AbortSignal) => api.get<PaginatedResponse<Pick<Customer, 'id' | 'code' | 'name' | 'status' | 'phone' | 'contactPhone'>>>('/sales/customers/lookup', { params: filters as any, signal }),
  getLookupById: (id: string, signal?: AbortSignal, purpose?: string) => api.get<Pick<Customer, 'id' | 'code' | 'name' | 'status' | 'phone' | 'contactPhone'>>(`/sales/customers/lookup/${id}`, { params: purpose ? { purpose } : undefined, signal }),
